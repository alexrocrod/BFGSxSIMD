...
PostProcess::PostProcess(
    ...
    : portfolio_(portfolio), nettingSetManager_(nettingSetManager), market_(market), cube_(cube),
      ...
      kvaTheirCvaRiskWeight_(kvaTheirCvaRiskWeight), use_aadc_(use_aadc) {
    ...
    Size samples = use_aadc_ ? 1 : cube->samples();
    ...
    for (Size i = 0; i < portfolio->size(); ++i) {
        ...
        for (Size j = 0; j < dates; ++j) {
            ...
            for (Size k = 0; k < samples; ++k) {
                Real npv = d > nextBreakDate && exerciseNextBreak ? Real(0.0) : cube->get(i, j, k);
            ...
            Size index = Size2(stdpassive::floor(quantile_ * (samples - 1) + 0.5));
        ...
        if (t > 0) {
            vector<Real> weights(t);
            weights[0] = times[0];
            for (Size k = 1; k < t; k++)
                weights[k] = times[k] - times[k - 1];
            Real totalWeights = std::accumulate(weights.begin(), weights.end(), Real(0.0));
            for (Size k = 0; k < t; k++)
...
void PostProcess::updateNettingSetKVA() {
	...
	DLOG("Our KVA-CCR " << nettingSetId << ": PD=" << PD1);
	    ...
	    eepe_kva_1 = count > 0 ? eepe_kva_1/sumdt : Real(0.0);
	    eepe_kva_2 = count > 0 ? eepe_kva_2/sumdt : Real(0.0);

	    // KVA CCR using the IRB risk weighted asset method and IMM:
	    // KVA effective maturity of the nettingSet, capped at 5
	    Real kvaNWMaturity1 = std::min(1.0 + (effMatDenom1 == 0.0 ? Real(0.0) : effMatNumer1 / effMatDenom1), 5.0);
	    Real kvaNWMaturity2 = std::min(1.0 + (effMatDenom2 == 0.0 ? Real(0.0): effMatNumer2 / effMatDenom2), 5.0);
	    ...
	    Real kvaCvaMaturity1 = 1.0 + (effMatDenom1 == 0.0 ? Real(0.0) : effMatNumer1 / effMatDenom1);
	    Real kvaCvaMaturity2 = 1.0 + (effMatDenom2 == 0.0 ? Real(0.0) : effMatNumer2 / effMatDenom2);
...
void PostProcess::updateStandAloneXVA() {
    ...
    for (Size i = 0; i < trades; ++i) {
        ...
        for (Size j = 0; j < dates; ++j) {
            ...
            Real dvaS0 = dvaDts.empty() ? Real(1.0) : dvaDts->survivalProbability(d0);
            Real dvaS1 = dvaDts.empty() ? Real(1.0) : dvaDts->survivalProbability(d1);
...
void PostProcess::dynamicInitialMargin() {
    ...
    Size simple_dim_index_h = Size(stdpassive::floor(dimQuantile_ * (samples - 1) + 0.5));
    Size simple_dim_index_p = Size(stdpassive::floor((1.0 - dimQuantile_) * (samples - 1) + 0.5));
    ...
    for (auto n : nettingSets) {
        ...
        for (Size j = 0; j < dates - 1; ++j) {
            accumulator_set<Real, stats<tag::mean, tag::variance>> accDiff;
            accumulator_set<Real, stats<tag::mean>> accOneOverNumeraire;
            for (Size k = 0; k < samples; ++k) {
...
void PostProcess::performT0DimCalc() {
    ...
    Real confidenceLevel = QuantLib::InverseCumulativeNormal()(dimQuantile_);
    Size simple_dim_index_h = Size(stdpassive::floor(dimQuantile_ * (cube_->samples() - 1) + 0.5));
    for (auto it_map = nettingSetNPV_.begin(); it_map != nettingSetNPV_.end(); ++it_map) {
        ...
        accumulator_set<Real, stats<tag::mean, tag::variance>> acc_delMtm;
        accumulator_set<Real, stats<tag::mean>> acc_OneOverNum;

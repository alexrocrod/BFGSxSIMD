...
public:
    ...
    //! Enable AADC for NPV cube construction
    void enableAADC() {use_aadc_ = true;}
...
//    typedef __m256d mmType;
    ...
    typedef __m256d mmType;
#endif
    std::shared_ptr<aadc::AADCFunctions<mmType>>& aadFuncs() {return aad_funcs_;}
    std::map<std::tuple<string, YieldCurveType, std::string>, AADCCurveData> curves_data_;
    std::vector<std::vector<aadc::AADCResult> > trade_res_;
    bool useAADC() {return use_aadc_;}

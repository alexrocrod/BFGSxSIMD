...
public:
    ...
    ScenarioSimMarketParameters()
        : extrapolate_(false), swapVolIsCube_(false), swapVolSimulateATMOnly_(true), swapVolStrikeSpreads_({Real(0.0)}),
          fxVolIsSurface_(false), fxMoneyness_({Real(0.0)}), equityIsSurface_(false), equityVolSimulateATMOnly_(true),
          equityMoneyness_({Real(1.0)}), cprSimulate_(false), correlationIsSurface_(false), correlationStrikes_({Real(0.0)}) {
        // set defaults
        setDefaults();
    }

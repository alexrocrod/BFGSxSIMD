...
void ShiftScenarioGenerator::applyShift(Size i, Size j, Real shiftSize, bool up, ShiftType shiftType,
    ...
    Real tx = shiftX[i];
    Real ty = shiftY[j];
    Real tx1 = i > 0 ? shiftX[i - 1] : Real(QL_MAX_REAL);
    Real ty1 = j > 0 ? shiftY[j - 1] : Real(QL_MAX_REAL);
    Real tx2 = i < iMax ? shiftX[i + 1] : Real(-QL_MAX_REAL);
    Real ty2 = j < jMax ? shiftY[j + 1] : Real(-QL_MAX_REAL);

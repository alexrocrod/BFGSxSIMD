...
public:
    ...
    //AADC
    bool use_aadc_=false;
    int num_threads =1;
    int avx_debug_index_=0;
    int generator_shift_=0;
protected:
    ...
    //! AADC
    void runAADC(ValuationEngine& engine);
    //! generate NPV cube
...
#endif
    Real eps_=-0.0001;
    std::tuple<string, YieldCurveType, std::string> key_curve_bump_ 
        = std::make_tuple("xois_eur", YieldCurveType::Discount, "EUR");
    int bump_iter_=-1;
    std::map<std::tuple<string, YieldCurveType, std::string>, AADCCurveData> curves_data_;
    std::map<std::tuple<string, YieldCurveType, std::string>, aadc::VectorRes> curves_outargs_;
private:
    std::shared_ptr<aadc::AADCFunctions<mmType>> aad_funcs_;
    // End AADC
private:

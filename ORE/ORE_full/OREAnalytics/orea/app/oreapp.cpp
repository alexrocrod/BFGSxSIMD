...
int OREApp::run() {
    ...
    try {
        ...
        // if (use_aadc_) {
        //     aad_funcs_ = std::shared_ptr<aadc::AADCFunctions<mmType> >(
        //     new aadc::AADCFunctions<mmType>(
        //         {
        //             {aadc::AADC_UseCompressedCode, 1}
        //             , {aadc::AADC_CodeBufferSize, 0.25*1024*1024}
        //             , {aadc::AADC_NumCompressorThreads, 24}
        //             //, {aadc::AADC_ReuseConstants, 0}
        //             , {aadc::AADC_InitInputDiff, 1}   // reset input_diff
        //         }
        //     ));
        // }
        ...
        //             std::cout << setprecision(17) << postProcess_->nettingSetCVA(n) << "\n";
        //             delta = postProcess_->nettingSetCVA(n);
        //         }
        //         for (int i=0; i<10; i++) {
        //             if (bump_iter_>0) market_->shift(bump_iter_-1,  -1*eps_, key_curve_bump_);
        //             generateNPVCube();
        //             Settings::instance().evaluationDate() = asof_;
        //             runPostProcessor();
        //             for (auto n : postProcess_->nettingSetIds()) {
        //                 Real delta_up=postProcess_->nettingSetCVA(n);
        //                 std::cout << "Risk " << setprecision(10) <<  (delta_up - delta)/eps_ << " Corresponds to the " << i << "-th date from the end\n";
        //             }
...
void OREApp::buildNPVCube() {
    ...
    std::cout<<std::endl;
    o.str("");
    o << "Build Cube " << simPortfolio_->size() << " trades X " << grid_->size() << " dates X " << samples_ << " samples...";
    ...
    if (use_aadc_) engine.enableAADC();
    engine.buildCube(simPortfolio_, cube_, calculators);
    if (use_aadc_) {   // If AADC then calculate CVA (i.e. runPostProcessor()) after the first MC path, and not after the last one.
        runPostProcessor();
        runAADC(engine);
    }
    out_ << "OK" << endl;
...
void OREApp::generateNPVCube() {
    ...
    // AADC
    if (use_aadc_) {
        market_->saveYieldCurvesData(curves_data_);
        aad_funcs_->startRecording();
        market_->markYieldCurvesAsInput(curves_data_);
    }
    if (bump_iter_>=0) {  // Initital value of bump_iter_ is -1. During the bump&revalue loop
        market_->shift(bump_iter_, eps_, key_curve_bump_);   // entries of the curve with key key_curve_bump_ are consequently shocked
    }
    bump_iter_++;
    //!AADC
...
// AADC
void OREApp::runAADC(ValuationEngine& engine) {
#ifdef AADC_QL
    const auto& dates = grid_->dates();
    const auto& trades = simPortfolio_->trades();

    std::vector<aadc::AADCResult> CVAargs;
    aadc::AADCResult CVAarg;

    for (auto n : postProcess_->nettingSetIds()) {
        CVAarg=postProcess_->nettingSetCVA(n).markAsOutput();
        for (Size k = 0; k < trades.size(); ++k) {
            string tid = trades[k]->id();
            string nid = trades[k]->envelope().nettingSetId();
            if (nid != n) continue;
                CVAargs.push_back(postProcess_->tradeCVA(tid).markAsOutput());
        }
    }
    aad_funcs_->stopRecording();
    aad_funcs_->outStats(std::cout, "ORE");
    aad_funcs_->printPassiveExtractLocations(std::cout, "ORE");

    boost::timer rnd_timer;
    boost::shared_ptr<BrownianGenerator>& gen = MultiPathGeneratorSobolBrownianBridge::getBrownianGenerator();

    int avx_len(sizeof(mmType) / sizeof(double));
    int num_avx_paths(cube_->samples() % avx_len==0 ?  cube_->samples() / avx_len :1+cube_->samples() / avx_len);

    std::vector<Real> factors(gen->numberOfFactors());

    std::vector<aadc::mmVector<mmType>> randoms(num_avx_paths);
    int num_randoms;
    std::cout << "Number of required array inputs : "
        << (num_randoms = aad_funcs_->getNumberOfRequiredRandomVariables())
        << std::endl
    ;
   QL_REQUIRE(num_randoms == dates.size() * gen->numberOfFactors(), "Number of factors didn't match to the number of registered randoms");

    // Simulate random numbers using original Random Path Generator
    for (Size sample_avx = 0; sample_avx < num_avx_paths; ++sample_avx) {
        randoms[sample_avx].resize(num_randoms);
        for (Size avx_i = 0; avx_i < avx_len; ++avx_i) {
            Real weight = gen->nextPath();
            double* ptr = aadc::toDblPtr(randoms[sample_avx][0]) + avx_i;
            for (Size i = 0; i < dates.size(); ++i) {
                gen->nextStep(factors);
                for (Size f_i = 0; f_i < factors.size(); ++f_i) {
                    *ptr = AAD_PASSIVE(factors[f_i]);
                    ptr += avx_len;
                }
            }
        }
    ...
    // initialize cva_diff containers
    for (auto& item : curves_data_) item.second.CVA_diffs.resize(item.second.curve_args.size());

    LOG("ORE Generating randoms completed: " << setprecision(2) << rnd_timer.elapsed() << " sec ");
    std::cout << "ORE Generating randoms completed: " << setprecision(2) << rnd_timer.elapsed() << " sec \n";

    auto aadc_start = std::chrono::high_resolution_clock::now();

    std::mutex cube__mutex;
    auto threadWorker = [&] (
        Size sample_path_start
        , Size sample_path_end
        , std::vector<double>& cva
        , std::map<std::tuple<string, YieldCurveType, std::string>, AADCCurveData>& diff_container
    ){
        std::shared_ptr<aadc::AADCWorkSpace<mmType> > ws(aad_funcs_->createWorkSpace());
        ws->resetDiff();
        // aqui?
        for (auto& item : curves_data_) {
            std::cout << "\n Curve: " <<  std::endl;
            for (int j=0; j< item.second.curve_args.size(); j++) {
                double num=item.second.curve_vals[j].val;
                std::cout <<" : "<<  num << std::endl;
                //  If curve is marked as input then uncomment:
                ws->val(item.second.curve_args[j])=  aadc::mmSetConst<mmType>(num);
            }
        }

        for (Size sample_avx = sample_path_start; sample_avx < sample_path_end; ++sample_avx) {
            ws->resetDiff();
            ws->setArray(&(randoms[sample_avx].front()));
            aad_funcs_->forward(*ws,0,-1);

            {
                const std::lock_guard<std::mutex> lock(cube__mutex);
                for (int d_i = 0; d_i < dates.size(); ++d_i) {
                    for (int t_j = 0; t_j < trades.size(); ++t_j) {
                        for (int avx_i = 0; avx_i < avx_len; ++avx_i) {
                            Size sample = sample_avx * avx_len + avx_i;
                            if (sample < cube_->samples()) {
                                cube_->set(ws->valp(engine.trade_res_[d_i][t_j])[avx_i], t_j, d_i, sample);
                            }
                        }
                    }
                }
                for (int avx_i = 0; avx_i < avx_len; ++avx_i)   {
                    cva[0]+=ws->val(CVAarg)[avx_i]; // The zero component contains the total sum
                    for (int t_j = 0; t_j < trades.size(); ++t_j) {
                        cva[t_j+1]+=ws->val(CVAargs[t_j])[avx_i];
                    }
                }
            }
            ws->diff(CVAarg)=aadc::mmSetConst<mmType>(1.);
            aad_funcs_->reverse(*ws);

            // Collect diff data
            for (auto& item : diff_container) {
                for (int j=0; j< item.second.curve_args.size(); j++) {
                    item.second.CVA_diffs[j]+=aadc::mmSum(ws->diff(item.second.curve_args[j])) ;
                }
            }
        }
    };
    std::cout << "AADC loop num threads: " << num_threads << " \n";

    std::vector<std::unique_ptr<std::thread>> threads(num_threads);
    int num_avx_samples_per_thread(num_avx_paths / num_threads);
    std::vector<std::vector<double>> cva_threads(num_threads,std::vector<double>(trades.size()+1,0.));

    std::vector<std::map<std::tuple<string, YieldCurveType, std::string>, AADCCurveData>>
        cva_diffs(num_threads, curves_data_)
    ;

    for (int th_i = 0; th_i < num_threads; ++th_i) {
        threads[th_i] = std::unique_ptr<std::thread>(
            new std::thread(
                threadWorker
                , th_i * num_avx_samples_per_thread
                , (th_i == num_threads - 1) ? num_avx_paths : (th_i + 1) * num_avx_samples_per_thread
                , std::ref(cva_threads[th_i])
                , std::ref(cva_diffs[th_i])
            )
        );
    }
    for(auto&& t: threads) t->join();
    engine.updateProgress(cube_->samples(), cube_->samples());

    auto aadc_stop= std::chrono::high_resolution_clock::now();
    std::chrono::microseconds aadc_time = std::chrono::duration_cast<std::chrono::microseconds>(
        aadc_stop - aadc_start
    );
    std::vector<double> cva(cva_threads[0].size(), 0.);
    for (int i=0; i<num_threads; i++) {
        for (int j=0; j<cva_threads[0].size(); j++) {
            cva[j] +=cva_threads[i][j];
        }
    }
    for (int j=0; j<cva.size(); j++) cva[j]/=cube_->samples();

    // Write CVA data into the porstProcess_
    for (auto n : postProcess_->nettingSetIds()) {
        postProcess_->nettingSetCVA(n)=cva[0];
        std::cout << "\nTotal CVA: " <<  setprecision(10) << cva[0] << std::endl;
        for (Size k = 0; k < trades.size(); ++k) {
            string tid = trades[k]->id();
            string nid = trades[k]->envelope().nettingSetId();
            if (nid != n) continue;
            postProcess_->tradeCVA(tid)=cva[k+1];
            std::cout << setprecision(17) << tid << std::string(max(1, int(30-tid.size())), ' ') << postProcess_->tradeCVA(tid) << std::endl;
        }
    }

    for (auto& item : curves_data_) {
        if (item.first != key_curve_bump_) continue;
        std::cout << "\n\nRisks: CurveKey=(" << std::get<0>(item.first)
            << ", YieldCurveType::" << int(std::get<1>(item.first))
            << ", "<< std::get<2>(item.first)<< ")\n"
        ;
        for (int j=0; j< item.second.curve_args.size(); j++) {
            double diff=cva_diffs[0][item.first].CVA_diffs[j];
            for (int th_i=1; th_i<num_threads; th_i++) {
                diff+=cva_diffs[th_i][item.first].CVA_diffs[j];
            }
            item.second.CVA_diffs[j]=diff/cube_->samples();
            std::cout << item.second.CVA_diffs[j] << " ";
        }
    }
    std::cout << "\n\nAADC ValuationEngine completed: loop " << setprecision(5) << aadc_time.count()/1000000.0 << " sec\n";
    LOG("AADC ValuationEngine completed: loop " << setprecision(2) << aadc_time.count()/1000000.0 << " sec ");
#endif
}
//!AADC
...
void OREApp::runPostProcessor() {
    ...
    kvaOurCvaRiskWeight, kvaTheirCvaRiskWeight, use_aadc_ ? true : false);
}

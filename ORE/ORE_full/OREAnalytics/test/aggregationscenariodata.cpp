...
BOOST_AUTO_TEST_CASE(testInMemoryAggregationScenarioData) {
    ...
    for (Size i = 0; i < 3; ++i) {
        for (Size j = 0; j < 5; ++j) {
            BOOST_CHECK_CLOSE_PASSIVE(data.get(i, j, AggregationScenarioDataType::IndexFixing, "OIS_EUR"),
                              0.0001 * i + 0.01 * j, tol);
            BOOST_CHECK_CLOSE_PASSIVE(data.get(i, j, AggregationScenarioDataType::IndexFixing, "OIS_USD"),
                              0.1 + 0.0001 * i + 0.01 * j, tol);
            BOOST_CHECK_CLOSE_PASSIVE(data.get(i, j, AggregationScenarioDataType::IndexFixing, "OIS_GBP"),
                              0.2 + 0.0001 * i + 0.01 * j, tol);
            BOOST_CHECK_CLOSE_PASSIVE(data.get(i, j, AggregationScenarioDataType::FXSpot, "EURUSD"), i + 0.1 * j, tol);
            BOOST_CHECK_CLOSE_PASSIVE(data.get(i, j, AggregationScenarioDataType::FXSpot, "EURGBP"), 2.0 + i + 0.1 * j, tol);
        }
    }
}

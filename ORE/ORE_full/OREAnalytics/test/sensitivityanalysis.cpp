...
BOOST_AUTO_TEST_CASE(testEquityOptionDeltaGamma) {
    ...
    for (auto it : qlInfoMap) {
        ...
        BOOST_CHECK_CLOSE_PASSIVE(res.equityVolDelta, qlInfo.vega * qlInfo.fx * (bp * eqVol), tol);

        BOOST_CHECK_CLOSE_PASSIVE(res.equitySpotDelta, qlInfo.delta * qlInfo.fx * (bp * qlInfo.spot), tol);
        BOOST_CHECK_CLOSE_PASSIVE(res.equitySpotGamma, qlInfo.gamma * qlInfo.fx * (pow(bp * qlInfo.spot, 2)), tol);
    }
}
...
BOOST_AUTO_TEST_CASE(testFxOptionDeltaGamma) {
    ...
    for (Size i = 0; i < portfolio->size(); ++i) {
        ...
        BOOST_CHECK_CLOSE_PASSIVE(info.fx, info.baseNpv / info.qlNpv, 0.01);
        qlInfoMap[info.id] = info;
    }
    ...
    for (auto it : qlInfoMap) {
        ...
        BOOST_CHECK_CLOSE_PASSIVE(res.domDiscountDelta, qlInfo.rho * qlInfo.fx * bp, tol);
        BOOST_CHECK_CLOSE_PASSIVE(res.forDiscountDelta, qlInfo.divRho * qlInfo.fx * bp, tol);
        Real fxVol = initMarket->fxVol(qlInfo.forCcy + qlInfo.domCcy)
                         ->blackVol(1.0, 1.0, true); // TO-DO more appropriate vol extraction
        BOOST_CHECK_CLOSE_PASSIVE(res.fxVolDelta, qlInfo.vega * qlInfo.fx * (bp * fxVol), tol);
        if (res.hasFxSpotDomSensi) {
            ...
            BOOST_CHECK_CLOSE_PASSIVE(res.fxSpotDeltaDom, qlInfo.delta * qlInfo.fx * (bp * qlInfo.trnFx), tol);
            BOOST_CHECK_CLOSE_PASSIVE(res.fxSpotGammaDom, qlGamma * qlInfo.fx * (pow(bp * res.fxRateSensiDom, 2)), tol);
        }
        if (res.hasFxSpotForSensi) {
            ...
            BOOST_CHECK_CLOSE_PASSIVE(res.fxSpotDeltaFor, qlInfo.delta * qlInfo.fx * (-bp * qlInfo.trnFx), tol);
            BOOST_CHECK_CLOSE_PASSIVE(res.fxSpotGammaFor, qlGamma * qlInfo.fx * (pow(-bp * res.fxRateSensiFor, 2)), tol);
        }
    }
    ObservationMode::instance().setMode(backupMode);
    IndexManager::instance().clearHistories();
            ...
            if (s.type() == ShiftScenarioGenerator::ScenarioDescription::Type::Cross) {
                ...
                if (fabs(crossgamma) >= threshold) {
                    ...
                    if (cached_it != cachedMap.end()) {
                        ...
                        BOOST_CHECK_CLOSE_PASSIVE(crossgamma, cached_cg, rel_tol);
                        count++;
                    }
                }
            }

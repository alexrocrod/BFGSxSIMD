...
boost::shared_ptr<Portfolio> buildPortfolio(Size portfolioSize, boost::shared_ptr<EngineFactory>& factory) {
    ...
    for (Size i = 0; i < portfolioSize; i++) {
        ...
        LegData fixedLeg(boost::make_shared<FixedLegData>(vector<Real>(1, fixedRate)), isPayer, ccy, fixedSchedule,
                         fixDC, notional);

        // float Leg
        vector<Real> spreads(1, 0);
...
void test_performance(Size portfolioSize, ObservationMode::Mode om, double nonZeroPVRatio, vector<Real>& epe_archived,
    ...
    std::map<std::pair<std::string, std::string>, double> corr;
    ...
    // check results
    BOOST_CHECK_CLOSE_PASSIVE(nonZeroPVRatio, nonZeroPerc, 0.005);

    for (Size i = 0; i < epe_archived.size(); ++i) {
        BOOST_CHECK_CLOSE_PASSIVE(eeVec[i], epe_archived[i], 0.01);
    }

    for (Size i = 0; i < ene_archived.size(); ++i) {
        BOOST_CHECK_CLOSE_PASSIVE(eneVec[i], ene_archived[i], 0.01);
    }
}

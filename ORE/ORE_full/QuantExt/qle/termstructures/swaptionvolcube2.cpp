...
boost::shared_ptr<SmileSection> SwaptionVolCube2::smileSectionImpl(Time optionTime, Time swapLength) const {
    ...
    Rounding rounder(0);
    Period swapTenor(static_cast<Integer>(AAD_PASSIVE(rounder(swapLength * 12.0))), Months);
...
boost::shared_ptr<SmileSection> SwaptionVolCube2::smileSectionImpl(const Date& optionDate,
    ...
    Volatility referenceVol = volsAreSpreads_ ? atmVol_->volatility(optionDate, swapTenor, atmForward) : Real(0.0);

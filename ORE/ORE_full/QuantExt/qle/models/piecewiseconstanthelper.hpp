...
inline void PiecewiseConstantHelper1::update() const {
    ...
    for (Size i = 0; i < t_.size(); ++i) {
        sum += direct(y_->params()[i]) * direct(y_->params()[i]) * (t_[i] - (i == 0 ? Real(0.0) : t_[i - 1]));
        b_[i] = sum;
    }
}
...
inline void PiecewiseConstantHelper2::update() const {
    ...
    c_.resize(t_.size());
    for (Size i = 0; i < t_.size(); ++i) {
        Real t0 = (i == 0 ? Real(0.0) : t_[i - 1]);
        sum += direct(y_->params()[i]) * (t_[i] - t0);
        b_[i] = sum;
        Real b2Tmp = (i == 0 ? Real(0.0) : b_[i - 1]);
...
inline void PiecewiseConstantHelper3::update() const {
    ...
    std::vector<Real>::const_iterator end = std::unique(tTmp.begin(), tTmp.end(), std::ptr_fun(close_enough_passive));
    ...
    for (Size i = 0; i < tUnion_.size() + 1; ++i) {
        ...
        Real t = (i == tUnion_.size() ? (tUnion_.size() == 0 ? Real(1.0) : tUnion_.back() + 1.0)
                                      : (0.5 * (tUnion_[i] + (i > 0 ? tUnion_[i - 1] : Real(0.0)))));
    ...
    for (Size i = 0; i < tUnion_.size(); ++i) {
        Real t0 = (i == 0 ? Real(0.0) : tUnion_[i - 1]);

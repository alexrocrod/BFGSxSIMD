...
public:
 ...
 void calibrateVolatilitiesIterative(  int& aadc_i_ 
...
inline void LinearGaussMarkovModel::calibrateVolatilitiesIterative(
    ...
    for (Size i = 0; i < helpers.size(); ++i) {
        std::vector<boost::shared_ptr<BlackCalibrationHelper> > h(1, helpers[i]);
        if (i!=10)  calibrate(h, method, endCriteria, constraint, weights, MoveVolatility(i));
            else {
                calibrate( aadc_i_ 
                    ,  curve
                    , use_aadc_
                    ...
                    , curve_args
                    ,
                    h, method, endCriteria, constraint, weights, MoveVolatility(i)); 
            }

        /*
        if (i== 10) {  //9-nonsus. 10-suc Nado idti vnutr desyatogo
            Real eps=0.000001;
            if (use_aadc_) {
                    idouble::CheckPoint();

...
public:
    ...
    virtual Real value(const Array& params) const {
        ...
        value=std::max(1e-6, value); // Zhenya
    ...
    virtual Disposable<Array> valuesA(int& aadc_i_ 
...
void LinkableCalibratedModel::calibrate(int& aadc_i_ 
                                        , Handle<YieldTermStructure> curve
                                        , bool use_aadc_
                                        , bool shift_
                                        ,  aadc::VectorArg& curve_args
    ...
    calibrate( aadc_i_ 
                    ...
                    , curve_args
                    ,
        tmp, method, endCriteria, additionalConstraint, weights, fixParameters);
    /*
    Real eps=0.000001;
    if (use_aadc_) {
            idouble::CheckPoint();
            curve->markVecAsInput(curve_args);
    }
    if (aadc_i_>=0 && shift_) {
        curve->data()[curve->data().size()-aadc_i_-1]+=eps;
        curve->CurveUpdate();
    }
    if (shift_) aadc_i_++;
    */
 ...
 void LinkableCalibratedModel::calibrate(int& aadc_i_ 
    ...
    */
    ...
    endCriteria_ = method.minimize(aadc_i_ 
                                    ,  curve
                                    , use_aadc_
                                    , shift_
                                    , curve_args

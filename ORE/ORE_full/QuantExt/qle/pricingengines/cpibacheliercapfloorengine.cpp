...
void CPIBachelierCapFloorEngine::calculate() const {
    ...
    Real K = std::pow(1.0 + arguments_.strike, timeFromStart);
    ...
    // for strikeRate(t):
    Real strikeZeroRate =
        std::pow(arguments_.baseCPI / baseFixing * std::pow(1.0 + arguments_.strike, timeFromStart), 1.0 / timeFromBase) - 1.0;

...
Disposable<Array> CrossAssetStateProcess::drift(Time t, const Array& x) const {
    ...
    boost::unordered_map<double, Array>::const_iterator i = cache_m_.find(AAD_PASSIVE(t));
    if (i == cache_m_.end()) {
        /* z0 has drift 0 */
        for (Size i = 1; i < n; ++i) {
        ...
        for (Size k = 0; k < n_eq; ++k) {
            ...
            Real sigmaxi = (i == 0) ? Real(0.0): model_->fxbs(i - 1)->sigma(t);
            // ir-eq corr
            // Real rhozsik = model_->correlation(EQ, k, IR, i); // eq cur
            Real rhozs0k = model_->correlation(EQ, k, IR, 0); // base cur
            // fx-eq corr
            Real rhoxsik = (i == 0) ? Real(0.0) : // no fx process for base-ccy
        ...
        cache_m_.insert(std::make_pair(AAD_PASSIVE(t), res));
    } else {
        res = i->second;
    }
...
Disposable<Matrix> CrossAssetStateProcess::diffusion(Time t, const Array& x) const {
    boost::unordered_map<double, Matrix>::const_iterator i = cache_d_.find(AAD_PASSIVE(t));
    if (i == cache_d_.end()) {
        Matrix tmp = pseudoSqrt(diffusionImpl(t, x), salvaging_);
        cache_d_.insert(std::make_pair(AAD_PASSIVE(t), tmp));
        return tmp;
    } else {
...
Disposable<Array> CrossAssetStateProcess::ExactDiscretization::drift(const StochasticProcess& p, Time t0,
    ...
    Array res;
    cache_key k = { AAD_PASSIVE(t0), AAD_PASSIVE(dt) };

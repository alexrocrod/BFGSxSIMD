...
public:
    explicit integrand_f(const LognormalCmsSpreadPricer* pricer) : pricer(pricer) {}
...
LognormalCmsSpreadPricer::LognormalCmsSpreadPricer(const boost::shared_ptr<CmsCouponPricer> cmsPricer,
    ...
    if (volatilityType == boost::none) {
        ...
        shift1_ = shift1 == Null<Real>() ? Real(0.0): shift1;
        shift2_ = shift2 == Null<Real>() ? Real(0.0) : shift2;
...
Real LognormalCmsSpreadPricer::integrand(const Real x) const {
    ...
    Real v = M_SQRT2 * x;
    Real h = k_ - b_ * s2_ * std::exp((m2_ - 0.5 * v2_ * v2_) * fixingTime_ + v2_ * std::sqrt(fixingTime_) * v);
    Real phi1, phi2;
    phi1 = (*cnd_)(phi_ *
                   (std::log(a_ * s1_ / h) + (m1_ + (0.5 - rho() * rho()) * v1_ * v1_) * fixingTime_ +
                    rho() * v1_ * std::sqrt(fixingTime_) * v) /
                   (v1_ * std::sqrt(fixingTime_ * (1.0 - rho() * rho()))));
    phi2 = (*cnd_)(
        phi_ *
        (std::log(a_ * s1_ / h) + (m1_ - 0.5 * v1_ * v1_) * fixingTime_ + rho() * v1_ * std::sqrt(fixingTime_) * v) /
        (v1_ * std::sqrt(fixingTime_ * (1.0 - rho() * rho()))));
    Real f = a_ * phi_ * s1_ *
                 std::exp(m1_ * fixingTime_ - 0.5 * rho() * rho() * v1_ * v1_ * fixingTime_ +
                          rho() * v1_ * std::sqrt(fixingTime_) * v) *
                 phi1 -
             phi_ * h * phi2;
...
Real LognormalCmsSpreadPricer::integrand_normal(const Real x) const {
    ...
    Real beta = phi_ * (gearing1_ * adjustedRate1_ + gearing2_ * adjustedRate2_ - k_ +
                        std::sqrt(fixingTime_) * (rho() * gearing1_ * vol1_ + gearing2_ * vol2_) * s);
    Real f = close_enough(alpha_, 0.0)
                 ? std::max(beta, 0.0)
                 : psi_ * alpha_ / (M_SQRTPI * M_SQRT2) * std::exp(-beta * beta / (2.0 * alpha_ * alpha_)) +
...
void LognormalCmsSpreadPricer::initialize(const FloatingRateCoupon& coupon) {
    ...
    discount_ =
        paymentDate_ > couponDiscountCurve_->referenceDate() ? couponDiscountCurve_->discount(paymentDate_) : Real(1.0);
    ...
    if (fixingDate_ > today_) {
        ...
        if (inheritedVolatilityType_ && volType_ == ShiftedLognormal) {
            ...
            shift2_ = swvol->shift(fixingDate_, index_->swapIndex2()->tenor());
        }

        if (swcub == NULL) {
            // not a cube, just an atm surface given, so we can
            // not easily convert volatilities and just forbid it
            QL_REQUIRE(inheritedVolatilityType_, "if only an atm surface is given, the volatility "
                                                 "type must be inherited");
            vol1_ = swvol->volatility(fixingDate_, index_->swapIndex1()->tenor(), swapRate1_);
            vol2_ = swvol->volatility(fixingDate_, index_->swapIndex2()->tenor(), swapRate2_);
        } else {
            vol1_ = swcub->smileSection(fixingDate_, index_->swapIndex1()->tenor())
                        ->volatility(swapRate1_, volType_, shift1_);
            vol2_ = swcub->smileSection(fixingDate_, index_->swapIndex2()->tenor())
                        ->volatility(swapRate2_, volType_, shift2_);
        }

        if (volType_ == ShiftedLognormal) {
            mu1_ = 1.0 / fixingTime_ * std::log((adjustedRate1_ + shift1_) / (swapRate1_ + shift1_));
            mu2_ = 1.0 / fixingTime_ * std::log((adjustedRate2_ + shift2_) / (swapRate2_ + shift2_));
        }
...
Real LognormalCmsSpreadPricer::optionletPrice(Option::Type optionType, Real strike) const {
    ...
    if (volType_ == ShiftedLognormal) {
        ...
        res += 1.0 / M_SQRTPI * (*integrator_)(integrand_f(this));
    } else {
        // normal volatility
        Real forward = gearing1_ * adjustedRate1_ + gearing2_ * adjustedRate2_;
        Real stddev =
...
Real LognormalCmsSpreadPricer::capletPrice(Rate effectiveCap) const {
    ...
    if (fixingDate_ <= today_) {
        ...
        const Rate Rs = std::max(coupon_->index()->fixing(fixingDate_) - effectiveCap, 0.);
...
Real LognormalCmsSpreadPricer::floorletPrice(Rate effectiveFloor) const {
    ...
    if (fixingDate_ <= today_) {
        // the fixing is determined
        const Rate Rs = std::max(effectiveFloor - coupon_->index()->fixing(fixingDate_), 0.);
        Rate price = gearing_ * Rs * coupon_->accrualPeriod() * discount_;
        return price;
    } else {
...
Real LognormalCmsSpreadPricer::swapletPrice() const {
    return gearing_ * coupon_->accrualPeriod() * discount_ * (gearing1_ * adjustedRate1_ + gearing2_ * adjustedRate2_) +
           spreadLegValue_;
}
} // namespace QuantExt

...
Disposable<std::vector<Real> > deltaGammaVarMc(const Matrix& omega, const Array& delta, const Matrix& gamma,
    ...
    Size cache = Size(stdpassive::floor(static_cast<double>(paths) * (1.0 - pmin) + 0.5)) + 2;
    ...
    for (Size i = 0; i < paths; ++i) {
        ...
        Array u = L * z;
        acc(AAD_PASSIVE(DotProduct(u, delta) + 0.5 * DotProduct(u, gamma * u)));
    }

    std::vector<Real> res;
    BOOST_FOREACH (Real q, p) {
        res.push_back(boost::accumulators::quantile(acc, boost::accumulators::quantile_probability = AAD_PASSIVE(q)));
    }

...
BOOST_AUTO_TEST_CASE(testVolatilitySurface) {
    ...
    for (Size i = 0; i < common.cStrikesUK.size(); i++) {
        ...
        for (Size j = 0; j < common.cfMaturitiesUK.size(); j++) {
            ...
            //            npv1);
            BOOST_CHECK_SMALL_AAD_PASSIVE(fabs(cached - npv1), 1e-10);
            ...
            BOOST_CHECK_SMALL_AAD_PASSIVE(fabs(cached - npv2), 1e-5); // depends on the default solver tolerance 1e-8
...
BOOST_AUTO_TEST_CASE(testPutCallParity) {
    ...
    for (Size i = 0; i < nStrike; ++i) {
        ...
        for (Size j = 0; j < nMat; ++j) {
            ...
            BOOST_CHECK_SMALL_AAD_PASSIVE(fabs(parity), 1.1); // bp upfront parity error
        }
    }
}

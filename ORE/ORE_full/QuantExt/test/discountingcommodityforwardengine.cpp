...
BOOST_AUTO_TEST_CASE(testPricing) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(forward->NPV(), quantity * (prices[1] - strike) * dfs[1], tolerance);
    ...
    forward = boost::make_shared<CommodityForward>(name, currency, position, quantity, maturity, strike);
    forward->setPricingEngine(engine);
    BOOST_CHECK_CLOSE_PASSIVE(forward->NPV(),
    ...
    forward->setPricingEngine(engine);
    BOOST_CHECK_CLOSE_PASSIVE(forward->NPV(), 0.0, tolerance);
    // Set includeReferenceDateEvents_ of Settings to true => value should be today's price - strike
    Settings::instance().includeReferenceDateEvents() = true;
    forward->recalculate();
    BOOST_CHECK_CLOSE_PASSIVE(forward->NPV(), -quantity * (prices[0] - strike), tolerance);
    ...
    BOOST_CHECK_CLOSE_PASSIVE(forward->NPV(), -quantity * (prices[1] - strike) * dfs[1] / npvDateDiscount, tolerance);
}

...
BOOST_AUTO_TEST_CASE(testBootstrap) {
    ...
    Real tol = 1e-5;
    BOOST_CHECK_SMALL_PASSIVE(swap->NPV(), tol);

    // Check fair spreads match. Bootstrap uses 1e-12 accuracy.
    Real relTol = 1e-10;
    BOOST_CHECK_CLOSE_PASSIVE(vars.spreadQuote->value(), swap->fairForeignSpread(), relTol);

    // Check the 5Y discount factor
    DiscountFactor expDisc = 0.91155524911218166;
    BOOST_CHECK_CLOSE_PASSIVE(expDisc, discCurve->discount(vars.asof + 5 * Years), relTol);
}
...
BOOST_AUTO_TEST_CASE(testSpotFxChange) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(vars.spotFxQuote->value(), std::fabs(vars.helper->swap()->leg(2).front()->amount()), relTol);

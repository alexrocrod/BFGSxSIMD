...
BOOST_AUTO_TEST_CASE(testSwapPricing) {
    ...
    BOOST_CHECK_SMALL_PASSIVE(swap->NPV() - expNpv, usdTolerance);

    Real expPayLegNpv = -12286.45;
    Real expPayLegBps = -2628.39;
    BOOST_CHECK_SMALL_PASSIVE(swap->legNPV(0) - expPayLegNpv, usdTolerance);
    BOOST_CHECK_SMALL_PASSIVE(swap->legBPS(0) - expPayLegBps, usdTolerance);
    BOOST_CHECK_SMALL_PASSIVE(swap->inCcyLegNPV(0) - expPayLegNpv * spotFx, usdTolerance * spotFx);
    BOOST_CHECK_SMALL_PASSIVE(swap->inCcyLegBPS(0) - expPayLegBps * spotFx, usdTolerance * spotFx);

    Real expRecLegNpv = 142064.36;
    Real expRecLegBps = 4735.03;
    BOOST_CHECK_SMALL_PASSIVE(swap->legNPV(1) - expRecLegNpv, usdTolerance);
    BOOST_CHECK_SMALL_PASSIVE(swap->legBPS(1) - expRecLegBps, usdTolerance);
    BOOST_CHECK_SMALL_PASSIVE(swap->inCcyLegNPV(1) - expRecLegNpv, usdTolerance);
    BOOST_CHECK_SMALL_PASSIVE(swap->inCcyLegBPS(1) - expRecLegBps, usdTolerance);

    BOOST_CHECK_SMALL_PASSIVE(0.253937551076 - swap->fairFixedRate(), 1e-10);
    BOOST_CHECK_SMALL_PASSIVE(-0.002740802104 - swap->fairSpread(), 1e-10);
}

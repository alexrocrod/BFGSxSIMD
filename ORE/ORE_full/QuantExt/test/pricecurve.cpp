...
template <class I> void commonChecks(CommonData& td, InterpolatedPriceCurve<I>& priceCurve, bool isLogLinear = false) {
    ...
    for (Size i = 0; i < td.times(0).size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(td.prices[i], priceCurve.price(td.times(0)[i], td.extrapolate), td.tolerance);
    }

    // Check the prices at the pillar dates
    for (Size i = 0; i < td.dates(0).size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(td.prices[i], priceCurve.price(td.dates(0)[i], td.extrapolate), td.tolerance);
    }
    ...
    for (Size i = 0; i < td.interpolationDates.size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(expResults[i], priceCurve.price(td.interpolationDates[i], td.extrapolate), td.tolerance);
        BOOST_CHECK_CLOSE_PASSIVE(expResults[i], priceCurve.price(td.interpolationTimes[i], td.extrapolate), td.tolerance);
    }
}
...
BOOST_AUTO_TEST_CASE(testTimesAndPricesCurve) {
    ...
    for (Size i = 0; i < td.interpolationTimes.size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(td.baseExpInterpResults[i], priceCurve.price(td.interpolationTimes[i], td.extrapolate),
                          td.tolerance);
        BOOST_CHECK_CLOSE_PASSIVE(td.afterExpInterpResults[i], priceCurve.price(td.interpolationDates[i], td.extrapolate),
                          td.tolerance);
    }
}
...
BOOST_AUTO_TEST_CASE(testTimesAndQuotesCurve) {
    ...
    td.updateQuotes();
    for (Size i = 0; i < td.interpolationTimes.size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(td.baseNewInterpResults[i], priceCurve.price(td.interpolationTimes[i], td.extrapolate),
                          td.tolerance);
        BOOST_CHECK_CLOSE_PASSIVE(td.afterNewInterpResults[i], priceCurve.price(td.interpolationDates[i], td.extrapolate),
                          td.tolerance);
    }
    ...
    for (Size i = 0; i < td.interpolationTimes.size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(td.baseNewInterpResults[i], priceCurve.price(td.interpolationTimes[i], td.extrapolate),
                          td.tolerance);
        BOOST_CHECK_CLOSE_PASSIVE(td.baseNewInterpResults[i], priceCurve.price(td.interpolationDates[i], td.extrapolate),
                          td.tolerance);
    }
}
...
BOOST_AUTO_TEST_CASE(testDatesAndPricesCurve) {
    ...
    for (Size i = 0; i < td.interpolationTimes.size(); i++) {
        BOOST_CHECK_CLOSE_PASSIVE(td.baseExpInterpResults[i], priceCurve.price(td.interpolationTimes[i], td.extrapolate),
                          td.tolerance);
        BOOST_CHECK_CLOSE_PASSIVE(td.baseExpInterpResults[i], priceCurve.price(td.interpolationDates[i], td.extrapolate),
                          td.tolerance);
    }
}
...
BOOST_AUTO_TEST_CASE(testNoTimeZeroWorks) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(15.1391304347826, priceCurve.price(0.25, td.extrapolate), td.tolerance);
    BOOST_CHECK_CLOSE_PASSIVE(13.5521739130435, priceCurve.price(0.0, td.extrapolate), td.tolerance);

    // Test log-linear interpolation also
    InterpolatedPriceCurve<LogLinear> logPriceCurve(times, td.prices, td.curveDayCounter);
    BOOST_CHECK_CLOSE_PASSIVE(15.331307232214800, logPriceCurve.price(0.25, td.extrapolate), td.tolerance);
    BOOST_CHECK_CLOSE_PASSIVE(14.054688467053400, logPriceCurve.price(0.0, td.extrapolate), td.tolerance);

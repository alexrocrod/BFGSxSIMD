        ...
        for (Size i = 0; i < optionTenors.size(); ++i) {
            for (Size j = 0; j < swapTenors.size(); ++j) {
                for (Size k = 0; k < strikeSpreads.size(); ++k) {
                    ...
                    Real nVolSpread =
                        k == atmStrikeIndex ?  Real(0.0) : nVolSpreadMin + rng.nextReal() * (nVolSpreadMax - nVolSpreadMin);
                    nVolSpreads[index][k] = Handle<Quote>(boost::shared_ptr<SimpleQuote>(new SimpleQuote(nVolSpread)));
                    Real lnVolSpread =
                        k == atmStrikeIndex ?  Real(0.0) : lnVolSpreadMin + rng.nextReal() * (lnVolSpreadMax - lnVolSpreadMin);

...
BOOST_AUTO_TEST_CASE(testInterpolatedCorrelationCurve) {
    ...
    Real tol = 1.0E-8;
    BOOST_CHECK_CLOSE_PASSIVE(interpCorr->correlation(1.5), 0.105, tol);
    BOOST_CHECK_CLOSE_PASSIVE(interpCorr->correlation(2.5), 0.115, tol);
    BOOST_CHECK_CLOSE_PASSIVE(interpCorr->correlation(3.5), 0.125, tol);
    BOOST_CHECK_CLOSE_PASSIVE(interpCorr->correlation(11), 0.18, tol);
}

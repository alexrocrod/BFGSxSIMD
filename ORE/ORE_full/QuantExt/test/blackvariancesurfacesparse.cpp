    ...
    for (auto vd: volData) {
        ...
        for (Size i = 1; i < vd.size(); i++) {
            ...
            if (expectedVol > 0.0001) {
                ...
                BOOST_CHECK_CLOSE_PASSIVE(vol, expectedVol, 1e-12);
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(testBlackVarianceSurfaceConstantVol) {
    ...
    for (Time t = 0.2; t < 20; t += 0.2) {
        for (Real strike = 1500; strike < 6000; strike += 100) {
            BOOST_CHECK_CLOSE_PASSIVE(surface.blackVol(t, strike), 0.1, 1e-12);
        }
    }
}
...
BOOST_AUTO_TEST_CASE(testBalckVarianceEdgeCases) { 
    ...
    // at reference date
    auto surface = QuantExt::BlackVarianceSurfaceSparse(today, cal, dates, strikes, vols, dc);
    BOOST_CHECK_CLOSE_PASSIVE(surface.blackVol(t, strike1), 0.1, 1e-12);
    BOOST_CHECK_CLOSE_PASSIVE(surface.blackVol(t, strike2), 0.1, 1e-12);
    BOOST_CHECK_CLOSE_PASSIVE(surface.blackVol(t, strike3), 0.1, 1e-12);
    BOOST_CHECK_CLOSE_PASSIVE(surface.blackVol(t, strike4), 0.1, 1e-12);

    // passed reference date
    Time t1 = surface.timeFromReference(Date(1, Mar, 2014));
    Real expectedVol = 0.1;
    BOOST_CHECK_CLOSE_PASSIVE(surface.blackVol(t1, strike2), expectedVol, 1e-12);

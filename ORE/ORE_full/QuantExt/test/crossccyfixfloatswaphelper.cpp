...
struct CommonVars {
    ...
    boost::shared_ptr<SimpleQuote> spotFx;
    ...
    boost::shared_ptr<CrossCcyFixFloatSwapHelper> helper;
...
BOOST_AUTO_TEST_CASE(testBootstrap) {
    ...
    Real absTol = 1e-5;
    BOOST_CHECK_SMALL_PASSIVE(swap->NPV(), absTol);

    // Check fair fixed rates match. Bootstrap uses 1e-12 accuracy.
    Real relTol = 1e-10;
    BOOST_CHECK_CLOSE_PASSIVE(vars.rate->value(), swap->fairFixedRate(), relTol);

    // Check the 5Y discount factor
    DiscountFactor expDisc = 0.3299260408883904;
    BOOST_CHECK_CLOSE_PASSIVE(expDisc, tryDiscCurve->discount(vars.asof + 5 * Years), relTol);
}
...
BOOST_AUTO_TEST_CASE(testSpotFxChange) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(vars.spotFx->value(), vars.helper->swap()->fixedNominal(), relTol);
...
BOOST_AUTO_TEST_CASE(testSpreadChange) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(vars.spread->value(), vars.helper->swap()->floatSpread(), relTol);

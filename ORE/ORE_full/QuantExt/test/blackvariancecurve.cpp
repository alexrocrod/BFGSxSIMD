...
BOOST_AUTO_TEST_CASE(testBlackVarianceCurve) {
    ...
    for (Size i = 0; i < times.size(); ++i) {
        BOOST_CHECK_CLOSE_PASSIVE(bvcTest.blackVol(times[i], strike), vols[i], 1e-12);
        BOOST_CHECK_CLOSE_PASSIVE(bvcTest.blackVol(dates[i], strike), vols[i], 1e-12);
    }

    // Now check that they give the same vols (including extrapolation)
    for (Time t = 0.1; t < numYears + 10.0; t += 0.1) {
        BOOST_CHECK_CLOSE_PASSIVE(bvcBase.blackVol(t, strike), bvcTest.blackVol(t, strike), 1e-12);
    }
    ...
    // and check again
    for (Time t = 0.1; t < numYears + 10.0; t += 0.1) {
        BOOST_CHECK_CLOSE_PASSIVE(bvcBase.blackVol(t, strike), 0.5 * bvcTest.blackVol(t, strike), 1e-12);
    }
}

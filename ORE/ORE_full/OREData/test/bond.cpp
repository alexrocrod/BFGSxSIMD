...
void checkNominalSchedule(const boost::shared_ptr<ore::data::Bond>& b, const std::vector<Real> notionals) {
    ...
    for (Size i = 0; i < notionals.size(); ++i)
        BOOST_CHECK_CLOSE_PASSIVE(bondNotionals[i], notionals[i], 1E-4);
}
...
BOOST_AUTO_TEST_CASE(testAmortizingBond) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(notional, expectedNotional, 1);
...
BOOST_AUTO_TEST_CASE(testMultiPhaseBond) {
    ...
    auto fixedLegRateData = boost::make_shared<FixedLegData>(vector<Real>(1, 0.01));

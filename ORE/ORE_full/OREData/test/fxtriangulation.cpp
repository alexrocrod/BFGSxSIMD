...
BOOST_AUTO_TEST_CASE(testValues) {
    ...
    // Check inverse
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("USDEUR")->value(), 1.0 / 1.0861, tol);
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("JPYEUR")->value(), 1.0 / 128.51, tol);

    // Check Triangulation
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("USDJPY")->value(), 128.51 / 1.0861, tol);
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("JPYUSD")->value(), 1.0861 / 128.51, tol);
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("USDGBP")->value(), 0.74519 / 1.0861, tol);
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("GBPUSD")->value(), 1.0861 / 0.74519, tol);
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("NOKSEK")->value(), 9.2640 / 9.6810, tol);

    // Check Triangulation where the EUR quote is reversed
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("ZZZUSD")->value(), 3.141 * 1.0861, tol);
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("USDZZZ")->value(), 1 / (3.141 * 1.0861), tol);
}
...
BOOST_AUTO_TEST_CASE(testMoreThanOneStep) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("EURNZD")->value(), 1.6450, tol);
    // then we should be able to get it with one step
    BOOST_CHECK_CLOSE_PASSIVE(fx.getQuote("USDNZD")->value(), 1.6450 / 1.0861, tol);
}

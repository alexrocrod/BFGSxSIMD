...
BOOST_AUTO_TEST_CASE(testEquityTradePrices) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(expectedNPV_Put, npv_put, 0.001);
    BOOST_CHECK_CLOSE_PASSIVE(expectedNPV_Put_Premium, npv_put_premium, 0.001);
    BOOST_CHECK_CLOSE_PASSIVE(npv_fwd, put_call_sum, 0.001);         // put-call parity check
    BOOST_CHECK_CLOSE_PASSIVE(npv_fwd, put_call_premium_sum, 0.001); // put-call parity check

...
public:
    TestMarket() {
        ...
        Handle<PriceTermStructure> priceCurve(
            boost::make_shared<InterpolatedPriceCurve<Linear>>(dates, prices, dayCounter));
...
BOOST_AUTO_TEST_CASE(testCommodityForwardTradeBuilding) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(commodityForward->quantity(), 100.0, testTolerance);
    BOOST_CHECK_EQUAL(commodityForward->maturityDate(), Date(19, Feb, 2019));
    BOOST_CHECK_CLOSE_PASSIVE(commodityForward->strike(), 1340.0, testTolerance);

    // Check the price (simple because DF = 1.0, 100 * (1348 - 1340))
    BOOST_CHECK_CLOSE_PASSIVE(commodityForward->NPV(), 800.0, testTolerance);
...
BOOST_AUTO_TEST_CASE(testCommodityForwardFromXml) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(commodityForward->strike(), 49.75, testTolerance);
    BOOST_CHECK_CLOSE_PASSIVE(commodityForward->quantity(), 500000.0, testTolerance);
}

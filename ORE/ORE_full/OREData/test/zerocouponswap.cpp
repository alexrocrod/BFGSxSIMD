...
BOOST_AUTO_TEST_CASE(testZeroCouponSwapPrice) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(market->discountCurve("GBP")->discount(today + 1 * Years), 0.9914, 0.0001);
    ...
    Real expectedNPV = -1 * notional.back() * (pow(1.0 + rate, Real(years)) - 1.0) * df;
    BOOST_CHECK_CLOSE_PASSIVE(swap->instrument()->NPV(), expectedNPV, 1E-8); // this is 1E-10 rel diff
}

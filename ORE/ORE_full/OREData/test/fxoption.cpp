...
BOOST_AUTO_TEST_CASE(testFXOptionPrice) {
    ...
    if (fxOption.npvCurrency() == "EUR") {
        BOOST_CHECK_CLOSE_PASSIVE(npv, expectedNPV_EUR, 0.2);
    } else if (fxOption.npvCurrency() == "USD") {
        BOOST_CHECK_CLOSE_PASSIVE(npv, expectedNPV_USD, 0.2);
        BOOST_CHECK_CLOSE_PASSIVE(npv_prem_usd, expectedNPV_USD_Premium_USD, 0.001);
        BOOST_CHECK_CLOSE_PASSIVE(npv_prem_eur, expectedNPV_USD_Premium_EUR, 0.001);
    } else {

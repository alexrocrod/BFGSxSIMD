...
BOOST_AUTO_TEST_CASE(testYoYCapFloor) {
    ...
    std::vector<Real> caps(1, 0.009);
    // Build capfloor trades
    boost::shared_ptr<Trade> yyCap(new ore::data::CapFloor(env, "Long", legYY, caps, std::vector<Real>()));
    ...
    BOOST_CHECK_CLOSE_PASSIVE(yyCap->instrument()->NPV(), qlCap->NPV(), 1E-8); // this is 1E-10 rel diff
}

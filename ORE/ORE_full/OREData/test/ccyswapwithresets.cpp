...
BOOST_AUTO_TEST_CASE(testCcySwapWithResetsPrice) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(market->discountCurve("EUR")->discount(today + 3 * Years), 1.006005, 0.0001);

    // Test USD discount(today+3*Years)) against base value
    BOOST_CHECK_CLOSE_PASSIVE(market->discountCurve("USD")->discount(today + 3 * Years), 0.96908, 0.0001);
    ...
    BOOST_CHECK_CLOSE_PASSIVE(std::abs(swap1->instrument()->NPV() - npvCCYswap), 250, 5);
    ...
    // check if difference between ore and bbg NPV does not exceed 250 USD (on 10M notional)
    BOOST_CHECK_CLOSE_PASSIVE(std::abs(swap2->instrument()->NPV() - npvCCYswapReset), 250, 5);

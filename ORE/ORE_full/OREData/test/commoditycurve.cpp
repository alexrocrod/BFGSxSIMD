...
BOOST_AUTO_TEST_CASE(testCommodityCurveConstruction) {
    ...
    // Check a few prices
    BOOST_CHECK_CLOSE_PASSIVE(priceCurve->price(asof), 1155.593, testTolerance);
    BOOST_CHECK_CLOSE_PASSIVE(priceCurve->price(Date(29, Feb, 2016)), 1157.8, testTolerance);
    BOOST_CHECK_CLOSE_PASSIVE(priceCurve->price(Date(28, Feb, 2017)), 1160.9, testTolerance);
    BOOST_CHECK_CLOSE_PASSIVE(priceCurve->price(Date(30, Jun, 2020)), 1189.5, testTolerance);

...
struct CommonVars {
    ...
    boost::shared_ptr<ore::data::Swap> makeDigitalCmsSpreadOption(bool call, vector<Real> strikes,
                                                                  vector<Real> payoffs) {
        ...
        if (call) {
            cmsLegData = boost::make_shared<DigitalCMSSpreadLegData>(
                boost::make_shared<CMSSpreadLegData>(
                    index1, index2, fixingdays, isinarrears, vector<Real>(), vector<string>(), vector<Real>(),
                    vector<string>(), vector<Real>(), vector<string>(), vector<Real>(), vector<string>(), false),
            ...
            cmsLegData = boost::make_shared<DigitalCMSSpreadLegData>(
                boost::make_shared<CMSSpreadLegData>(
                    index1, index2, fixingdays, isinarrears, vector<Real>(), vector<string>(), vector<Real>(),
                    vector<string>(), vector<Real>(), vector<string>(), vector<Real>(), vector<string>(), false),
                Position::Long, false, vector<Real>(), vector<string>(), vector<Real>(), vector<string>(),
    ...
    boost::shared_ptr<ore::data::Swap> makeCmsSpreadOption(vector<Real> spreads) {
        ScheduleData cmsSchedule(ScheduleRules(start, end, cmstenor, calStr, conv, conv, rule));

        vector<LegData> legData;
        CMSSpreadLegData cmsLegData(index1, index2, fixingdays, isinarrears, spreads, vector<string>(),
                                    vector<Real>(), vector<string>(), vector<Real>(), vector<string>(),
                                    vector<Real>(), vector<string>(), false);
    ...
    boost::shared_ptr<ore::data::Swap> makeCmsSpreadFloor(vector<Real> floors) {
        ScheduleData cmsSchedule(ScheduleRules(start, end, cmstenor, calStr, conv, conv, rule));

        vector<LegData> legData;
        CMSSpreadLegData cmsLegData(index1, index2, fixingdays, isinarrears, vector<Real>(), vector<string>(),
                                    vector<Real>(), vector<string>(), floors, vector<string>(), vector<Real>(),
                                    vector<string>(), false);
    ...
    boost::shared_ptr<ore::data::Swap> makeCmsSpreadCap(vector<Real> caps) {
        ScheduleData cmsSchedule(ScheduleRules(start, end, cmstenor, calStr, conv, conv, rule));

        vector<LegData> legData;
        CMSSpreadLegData cmsLegData(index1, index2, fixingdays, isinarrears, vector<Real>(), vector<string>(), caps,
                                    vector<string>(), vector<Real>(), vector<string>(), vector<Real>(),
                                    vector<string>(), false);
...
BOOST_AUTO_TEST_CASE(testDigitalCMSSpreadCoupon) {
...

    {
        ...
        boost::shared_ptr<ore::data::Swap> cmsDigitalSwapCall =
            vars.makeDigitalCmsSpreadOption(true, vector<Real>(1, strike), vector<Real>(1, pay));
        boost::shared_ptr<ore::data::Swap> cmsDigitalSwapPut =
            vars.makeDigitalCmsSpreadOption(false, vector<Real>(1, strike), vector<Real>(1, pay));
        boost::shared_ptr<ore::data::Swap> cmsSwap1 = vars.makeCmsSpreadOption(vector<Real>(1, pay));
        boost::shared_ptr<ore::data::Swap> cmsSwap2 = vars.makeCmsSpreadOption(vector<Real>(1, 0));
        ...
        BOOST_CHECK_CLOSE_PASSIVE(cmsDigitalSwapCall->instrument()->NPV(), cmsSwap2->instrument()->NPV(), 0.1);
        BOOST_CHECK_CLOSE_PASSIVE(cmsDigitalSwapPut->instrument()->NPV(), cmsSwap1->instrument()->NPV(), 0.1);
    }
    ...
    // check put cash-or-nothing payoffs
    {
        ...
        boost::shared_ptr<ore::data::Swap> cmsDigitalSwapPut =
            vars.makeDigitalCmsSpreadOption(false, vector<Real>(1, strike), vector<Real>(1, pay));
        boost::shared_ptr<ore::data::Swap> cmsSwap1 = vars.makeCmsSpreadFloor(vector<Real>(1, strike + eps / 2));
        boost::shared_ptr<ore::data::Swap> cmsSwap2 = vars.makeCmsSpreadFloor(vector<Real>(1, strike - eps / 2));
        cmsDigitalSwapPut->build(engineFactory);
        cmsSwap1->build(engineFactory);
        cmsSwap2->build(engineFactory);
        ...
        for (Size i = 0; i < leg.size(); i++) {
            ...
            Real r = pay * (frc1->rate() - frc2->rate()) / eps;

            BOOST_CHECK_CLOSE_PASSIVE(r, dc->putOptionRate(), 0.1);
        }
    }
    ...
    // check call cash-or-nothing payoffs
    {
        ...
        boost::shared_ptr<ore::data::Swap> cmsDigitalSwapCall =
            vars.makeDigitalCmsSpreadOption(true, vector<Real>(1, strike), vector<Real>(1, pay));
        boost::shared_ptr<ore::data::Swap> cmsSwap1 = vars.makeCmsSpreadCap(vector<Real>(1, strike + eps / 2));
        boost::shared_ptr<ore::data::Swap> cmsSwap2 = vars.makeCmsSpreadCap(vector<Real>(1, strike - eps / 2));
        cmsDigitalSwapCall->build(engineFactory);
        cmsSwap1->build(engineFactory);
        cmsSwap2->build(engineFactory);
        ...
        for (Size i = 0; i < leg.size(); i++) {
            ...
            BOOST_CHECK_CLOSE_PASSIVE(r, dc->callOptionRate(), 0.1);
        }
    }
    // check put asset-or-nothing payoffs
    {
        ...
        Real eps = 1e-4;
        boost::shared_ptr<ore::data::Swap> cmsDigitalSwapPut =
            vars.makeDigitalCmsSpreadOption(false, vector<Real>(1, strike), vector<Real>());
        boost::shared_ptr<ore::data::Swap> cmsSwap1 = vars.makeCmsSpreadFloor(vector<Real>(1, strike + eps / 2));
        boost::shared_ptr<ore::data::Swap> cmsSwap2 = vars.makeCmsSpreadFloor(vector<Real>(1, strike - eps / 2));
        boost::shared_ptr<ore::data::Swap> cmsSwap3 = vars.makeCmsSpreadFloor(vector<Real>(1, strike));
        boost::shared_ptr<ore::data::Swap> cmsSwap4 = vars.makeCmsSpreadFloor(vector<Real>());
        cmsDigitalSwapPut->build(engineFactory);
        cmsSwap1->build(engineFactory);
        cmsSwap2->build(engineFactory);
        ...
        for (Size i = 0; i < leg.size(); i++) {
            ...
            Real r = strike * (frc1->rate() - frc2->rate()) / eps;
            Real put = -frc4->rate() + frc3->rate();
            BOOST_TEST_MESSAGE(frc4->rate() << " " << frc3->rate() << " " << r << " " << put);

            BOOST_CHECK_CLOSE_PASSIVE(r - put, dc->putOptionRate(), 0.1);
        }
    }
    ...
    // check call asset-or-nothing payoffs
    {
        ...
        Real eps = 1e-4;
        boost::shared_ptr<ore::data::Swap> cmsDigitalSwapCall =
            vars.makeDigitalCmsSpreadOption(true, vector<Real>(1, strike), vector<Real>());
        boost::shared_ptr<ore::data::Swap> cmsSwap1 = vars.makeCmsSpreadCap(vector<Real>(1, strike + eps / 2));
        boost::shared_ptr<ore::data::Swap> cmsSwap2 = vars.makeCmsSpreadCap(vector<Real>(1, strike - eps / 2));
        boost::shared_ptr<ore::data::Swap> cmsSwap3 = vars.makeCmsSpreadCap(vector<Real>(1, strike));
        boost::shared_ptr<ore::data::Swap> cmsSwap4 = vars.makeCmsSpreadCap(vector<Real>());
        cmsDigitalSwapCall->build(engineFactory);
        cmsSwap1->build(engineFactory);
        cmsSwap2->build(engineFactory);
        cmsSwap3->build(engineFactory);
        ...
        for (Size i = 0; i < leg.size(); i++) {
            ...
            Real r = strike * (frc1->rate() - frc2->rate()) / eps;
            Real call = frc4->rate() - frc3->rate();

            BOOST_CHECK_CLOSE_PASSIVE(r + call, dc->callOptionRate(), 0.1);
        }
    }
}

...
BOOST_AUTO_TEST_CASE(testEquityCurve) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(1500.00, f_1y, 1.e-10); // hardcoded, to be the same as the input quote
    BOOST_CHECK_CLOSE_PASSIVE(1500.0, f_2y, 1.e-10);  // hardcoded, to be the same as the input quote
    ...
    BOOST_CHECK_CLOSE_PASSIVE(q_5y, q_50y, 1.e-10);
...
BOOST_AUTO_TEST_CASE(testEquityVolCurve) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(v_5y_atm, v_50y_atm, 1.e-10);
}
...
BOOST_AUTO_TEST_CASE(testCorrelationCurve) {
    ...
    LegData cms1YLeg(boost::make_shared<CMSSpreadLegData>(
                         ...
                         std::vector<Real>(1, fairSpread1Y), vector<string>(), vector<Real>(), vector<string>(),
                         vector<Real>(), vector<string>(), true),
    ...
    LegData cms2YLeg(boost::make_shared<CMSSpreadLegData>(
                         "USD-CMS-10Y", "USD-CMS-2Y", 2, true, std::vector<Real>(1, 0.0), vector<string>(),
                         std::vector<Real>(1, fairSpread2Y), vector<string>(), vector<Real>(), vector<string>(),
                         vector<Real>(), vector<string>(), true),
    ...
    BOOST_CHECK_SMALL_PASSIVE(npvCash - expectedNpv1Y, 0.000001);

    npvCash = cmsSpread2YCap.instrument()->NPV();

    BOOST_TEST_MESSAGE("NPV Cash 2Y             = " << npvCash);

    BOOST_CHECK_SMALL_PASSIVE(npvCash - expectedNpv2Y, 0.000001);
}

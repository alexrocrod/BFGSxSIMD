...
struct CommonVars {
    ...
    boost::shared_ptr<ore::data::Swap> makeSwap() {
        ...
        // build CMSSwap
        LegData fixedLegData(boost::make_shared<FixedLegData>(vector<Real>(1, fixedRate)), !isPayer, ccy,
    ...
    boost::shared_ptr<ore::data::Swap> makeSwap(Real fixedRate_, string fixtenor_) {
        ...
        // build CMSSwap
        LegData fixedLegData(boost::make_shared<FixedLegData>(vector<Real>(1, fixedRate_)), !isPayer, ccy,
                             fixedSchedule, fixDC, notionals);
        LegData cmsLegData(boost::make_shared<CMSLegData>(index, fixingdays, isinarrears, spread), isPayer, ccy,
                           cmsSchedule, fixDC, notionals);
    ...
    boost::shared_ptr<ore::data::Swap> makeCappedCmsLegSwap(vector<Real> caps,
    ...
    boost::shared_ptr<ore::data::CapFloor> makeCap(vector<Real> caps) {
        ScheduleData cmsSchedule(ScheduleRules(start, end, cmstenor, calStr, conv, conv, rule));
        ...
        boost::shared_ptr<ore::data::CapFloor> capfloor(
            new ore::data::CapFloor(env, longShort, cmsLegData, caps, vector<Real>()));
        return capfloor;
    }

    boost::shared_ptr<ore::data::Swap> makeFlooredCmsLegSwap(vector<Real> floors,
        ...
        LegData cmsLegData(boost::make_shared<CMSLegData>(index, fixingdays, isinarrears, spread, spreadDates,
                                                          vector<Real>(), vector<string>(), floors, floorDates),
                           isPayer, ccy, cmsSchedule, fixDC, notionals);
        legData.push_back(cmsLegData);
    ...
    boost::shared_ptr<ore::data::CapFloor> makeFloor(vector<Real> floors) {
        ScheduleData cmsSchedule(ScheduleRules(start, end, cmstenor, calStr, conv, conv, rule));
        ...
        Envelope env("CP1");
        boost::shared_ptr<ore::data::CapFloor> capfloor(
            new ore::data::CapFloor(env, longShort, cmsLegData, vector<Real>(), floors));
        return capfloor;
    }
...
BOOST_AUTO_TEST_CASE(testCMSAnalyticHagan) {
    ...
    BOOST_CHECK_CLOSE_PASSIVE(npv, expectedNPV, 1.0);
}
...
BOOST_AUTO_TEST_CASE(cmsCapFloor) {
    ...
    BOOST_CHECK_SMALL_PASSIVE(floorNpv, 0.01);

    BOOST_TEST_MESSAGE("Checking CMS Cap + CMS Floor = Swap...");
    vector<Real> floorRate(1, 0.021);
    ...
    BOOST_CHECK_CLOSE_PASSIVE(capFloorNpv, swapNpv, 1.0);
}

...
public:
    ...
    //! Constructor
    CashflowData(const vector<Real>& amounts, const vector<string>& dates)
        : LegAdditionalData("Cashflow", "CashflowData"), amounts_(amounts), dates_(dates) {}

    //! \name Inspectors
    //@{
    const vector<Real>& amounts() const { return amounts_; }
...
public:
    ...
    //! Constructor
    FixedLegData(const vector<Real>& rates, const vector<string>& rateDates = vector<string>())
        : LegAdditionalData("Fixed"), rates_(rates), rateDates_(rateDates) {}

    //! \name Inspectors
    //@{
    const vector<Real>& rates() const { return rates_; }
...
public:
    ...
    //! Constructor
    ZeroCouponFixedLegData(const vector<Real>& rates, const vector<string>& rateDates = vector<string>(), const string& compounding = "Compounded")
...
public:
    ...
    //! Constructor
    FloatingLegData(const string& index, QuantLib::Natural fixingDays, bool isInArrears, const vector<Real>& spreads,
                    const vector<string>& spreadDates = vector<string>(), const vector<Real>& caps = vector<Real>(),
                    const vector<string>& capDates = vector<string>(), const vector<Real>& floors = vector<Real>(),
                    const vector<string>& floorDates = vector<string>(),
                    const vector<Real>& gearings = vector<Real>(),
    ...
    const vector<Real>& spreads() const { return spreads_; }
    ...
    const vector<Real>& gearings() const { return gearings_; }
...
public:
    ...
    //! Constructor
    CPILegData(string index, Real baseCPI, string observationLag, bool interpolated, const vector<Real>& rates,
...
public:
    ...
    YoYLegData(string index, string observationLag, Size fixingDays,
               const vector<Real>& gearings = std::vector<Real>(),
               const vector<string>& gearingDates = std::vector<string>(),
               const vector<Real>& spreads = std::vector<Real>(),
               const vector<string>& spreadDates = std::vector<string>(),
               const vector<Real>& caps = vector<Real>(),
               const vector<string>& capDates = vector<string>(),
               const vector<Real>& floors = vector<Real>(),
               const vector<string>& floorDates = vector<string>(),
    ...
    const std::vector<Real>& gearings() const { return gearings_; }
    const std::vector<string>& gearingDates() const { return gearingDates_; }
    const std::vector<Real>& spreads() const { return spreads_; }
...
public:
    ...
    //! Constructor
    CMSLegData(const string& swapIndex, int fixingDays, bool isInArrears, const vector<Real>& spreads,
               const vector<string>& spreadDates = vector<string>(), const vector<Real>& caps = vector<Real>(),
               const vector<string>& capDates = vector<string>(), const vector<Real>& floors = vector<Real>(),
               const vector<string>& floorDates = vector<string>(), const vector<Real>& gearings = vector<Real>(),
...
public:
    ...
    CMSSpreadLegData(const string& swapIndex1, const string& swapIndex2, int fixingDays, bool isInArrears,
                     const vector<Real>& spreads, const vector<string>& spreadDates = vector<string>(),
                     const vector<Real>& caps = vector<Real>(), const vector<string>& capDates = vector<string>(),
                     const vector<Real>& floors = vector<Real>(),
                     const vector<string>& floorDates = vector<string>(),
                     const vector<Real>& gearings = vector<Real>(),
                     const vector<string>& gearingDates = vector<string>(), bool nakedOption = false)
...
public:
    ...
    DigitalCMSSpreadLegData(
        ...
        bool isCallATMIncluded = false, const vector<Real> callStrikes = vector<Real>(),
        const vector<string> callStrikeDates = vector<string>(), const vector<Real> callPayoffs = vector<Real>(),
        const vector<string> callPayoffDates = vector<string>(), Position::Type putPosition = Position::Long,
        bool isPutATMIncluded = false, const vector<Real> putStrikes = vector<Real>(),
        const vector<string> putStrikeDates = vector<string>(), const vector<Real> putPayoffs = vector<Real>(),
    ...
    const vector<Real> callStrikes() const { return callStrikes_; }
    const vector<Real> callPayoffs() const { return callPayoffs_; }
    ...
    const vector<Real> putStrikes() const { return putStrikes_; }
    const vector<Real> putPayoffs() const { return putPayoffs_; }
...
public:
    ...
    AmortizationData(string type, Real value, string startDate, string endDate, string frequency, bool underflow)
...
public:
    ...
    LegData(const boost::shared_ptr<LegAdditionalData>& innerLegData, bool isPayer, const string& currency,
            ...
            const std::vector<Real>& notionals = std::vector<Real>(),
            ...
            const string& foreignCurrency = "", const Real foreignAmount = 0, const string& fxIndex = "",
    ...
    const vector<Real>& notionals() const { return notionals_; }
...
Leg makeCMSLeg(const LegData& data, const boost::shared_ptr<QuantLib::SwapIndex>& swapindex,
               const boost::shared_ptr<EngineFactory>& engineFactory, const vector<Real>& caps = vector<Real>(),
               const vector<Real>& floors = vector<Real>(), const bool attachPricer = true);
...
vector<Real> buildScheduledVector(const vector<Real>& values, const vector<string>& dates,
                                    const Schedule& schedule);

// extend values to schedule size (if values is empty, the default value is used)
vector<Real> normaliseToSchedule(const vector<Real>& values, const Schedule& schedule,
                                   const Real defaultValue = Null<Real>());

// normaliseToSchedule concat buildScheduledVector
vector<Real> buildScheduledVectorNormalised(const vector<Real>& values, const vector<string>& dates,
                                              const Schedule& schedule, const Real defaultValue = Null<Real>());

// notional vector derived from a fixed amortisation amount
vector<Real> buildAmortizationScheduleFixedAmount(const vector<Real>& notionals, const Schedule& schedule,
                                                    const AmortizationData& data);

// notional vector with amortizations expressed as a percentage of initial notional
vector<Real> buildAmortizationScheduleRelativeToInitialNotional(const vector<Real>& notionals,
                                                                  const Schedule& schedule,
                                                                  const AmortizationData& data);

// notional vector with amortizations expressed as a percentage of the respective previous notional
vector<Real> buildAmortizationScheduleRelativeToPreviousNotional(const vector<Real>& notionals,
                                                                   const Schedule& schedule,
                                                                   const AmortizationData& data);

// notional vector derived from a fixed annuity amount
vector<Real> buildAmortizationScheduleFixedAnnuity(const vector<Real>& notionals, const vector<Real>& rates,

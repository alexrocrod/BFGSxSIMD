...
LegData::LegData(const boost::shared_ptr<LegAdditionalData>& concreteLegData, bool isPayer, const string& currency,
                 const ScheduleData& scheduleData, const string& dayCounter, const std::vector<Real>& notionals,
                 ...
                 const Real foreignAmount, const string& fxIndex, int fixingDays, const string& fixingCalendar,
...
Leg makeSimpleLeg(const LegData& data) {
    ...
    const vector<Real>& amounts = cashflowData->amounts();
...
Leg makeFixedLeg(const LegData& data) {
    ...
    vector<Real> rates = buildScheduledVector(fixedLegData->rates(), fixedLegData->rateDates(), schedule);
...
Leg makeZCFixedLeg(const LegData& data) {
    ...
    vector<Real> notionals = data.notionals();
    ...
    // (1 + r * dcf_0) * (1 + r * dcf_1)...
    Real totalDCF = 0;
    Real compoundFactor = 1;
    for (Size i = 0; i < dates.size() - 1; i++) {
        Real dcf = dc.yearFraction(dates[i], dates[i+1]);
    ...
    if (comp == QuantLib::Compounded)
        compoundFactor = std::pow(1.0 + fixedRate, totalDCF);
...
Leg makeIborLeg(const LegData& data, const boost::shared_ptr<IborIndex>& index,
    ...
    vector<Real> spreads = buildScheduledVectorNormalised(floatData->spreads(), floatData->spreadDates(), schedule, 0.0);
    vector<Real> gearings = buildScheduledVectorNormalised(floatData->gearings(), floatData->gearingDates(), schedule, 1.0);
    ...
    if (!data.amortizationData().empty()) {
        ...
        if (amortizationType == AmortizationType::Annuity) {
            ...
            Real annuity = data.amortizationData().front().value();
...
Leg makeCPILeg(const LegData& data, const boost::shared_ptr<ZeroInflationIndex>& index) {
    ...
    vector<Real> rates = buildScheduledVector(cpiLegData->rates(), cpiLegData->rateDates(), schedule);
    vector<Real> notionals = buildScheduledVector(data.notionals(), data.notionalDates(), schedule);
...
Leg makeYoYLeg(const LegData& data, const boost::shared_ptr<YoYInflationIndex>& index, const boost::shared_ptr<EngineFactory>& engineFactory) {
    ...
    vector<Real> gearings = buildScheduledVector(yoyLegData->gearings(), yoyLegData->gearingDates(), schedule);
    vector<Real> spreads = buildScheduledVector(yoyLegData->spreads(), yoyLegData->spreadDates(), schedule);
    vector<Real> notionals = buildScheduledVector(data.notionals(), data.notionalDates(), schedule);
...
Leg makeCMSLeg(const LegData& data, const boost::shared_ptr<QuantLib::SwapIndex>& swapIndex,
               const boost::shared_ptr<EngineFactory>& engineFactory, const vector<Real>& caps,
               const vector<Real>& floors, const bool attachPricer) {
    ...
    vector<Real> spreads = ore::data::buildScheduledVector(cmsData->spreads(), cmsData->spreadDates(), schedule);
    vector<Real> notionals = buildScheduledVector(data.notionals(), data.notionalDates(), schedule);
...
Leg makeDigitalCMSSpreadLeg(const LegData& data, const boost::shared_ptr<QuantLib::SwapSpreadIndex>& swapSpreadIndex,
    ...
    Real eps = 1e-4;
    vector<Real> callStrikes = ore::data::buildScheduledVector(digitalCmsSpreadData->callStrikes(),
    ...
    vector<Real> callPayoffs = ore::data::buildScheduledVector(digitalCmsSpreadData->callPayoffs(),
                                                                 digitalCmsSpreadData->callPayoffDates(), schedule);

    vector<Real> putStrikes = ore::data::buildScheduledVector(digitalCmsSpreadData->putStrikes(),
                                                                digitalCmsSpreadData->putStrikeDates(), schedule);
    vector<Real> putPayoffs = ore::data::buildScheduledVector(digitalCmsSpreadData->putPayoffs(),
    ...
    if (cmsSpreadData->gearings().size() > 0)
        digitalCmsSpreadLeg.withGearings(
            buildScheduledVector(cmsSpreadData->gearings(), cmsSpreadData->gearingDates(), schedule));
...
vector<Real> normaliseToSchedule(const vector<Real>& values, const Schedule& schedule, const Real defaultValue) {
    vector<Real> res = values;
...
vector<Real> buildAmortizationScheduleFixedAmount(const vector<Real>& notionals, const Schedule& schedule,
    ...
    vector<Real> nominals = normaliseToSchedule(notionals, schedule);
...
vector<Real> buildAmortizationScheduleRelativeToInitialNotional(const vector<Real>& notionals,
    ...
    bool underflow = data.underflow();
    Period amortPeriod = parsePeriod(data.frequency());
    Real fraction = data.value(); // first difference to "FixedAmount"
    QL_REQUIRE(fraction >= 0.0 && fraction <= 1.0,
               "amortization fraction " << fraction << " out of range"); // second difference to "FixedAmount"
    Real amort = fraction * nominals.front();                          // third difference to "FixedAmount"
    Date lastAmortDate = Date::minDate();
    for (Size i = 0; i < schedule.size() - 1; i++) {
        if (i > 0 && schedule[i] > lastAmortDate + amortPeriod - 4 * Days && schedule[i] >= startDate &&
            schedule[i] < endDate) { // FIXME: tolerance

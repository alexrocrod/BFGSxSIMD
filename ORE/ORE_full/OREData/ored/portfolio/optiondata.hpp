...
public:
    ...
    OptionData(string longShort, string callPut, string style, bool payoffAtExpiry, vector<string> exerciseDates,
               // notice days ?
               string settlement = "Cash", string settlementMethod = "", Real premium = 0, string premiumCcy = "",
               string premiumPayDate = "", vector<Real> exerciseFees = vector<Real>(),
               vector<Real> exercisePrices = vector<Real>())
    ...
    Real premium() const { return premium_; }
    const string& premiumCcy() const { return premiumCcy_; }
    const string& premiumPayDate() const { return premiumPayDate_; }
    const vector<Real>& exerciseFees() const { return exerciseFees_; }
    const vector<Real>& exercisePrices() const { return exercisePrices_; }
    //@}
...
private:
    ...
    vector<Real> exerciseFees_;
    vector<Real> exercisePrices_;
};
} // namespace data
} // namespace ore

...
void OptionWrapper::reset() {
    exercised_ = false;
    exerciseDate_ = Date().serialNumber();
}
...
//    if (!exercised_) {
        for (Size i = 0; i < effectiveExerciseDates_.size(); ++i) {
            if (today == effectiveExerciseDates_[i]) {
                exerciseDate_ = iIf(!exercised_ && exercise(), today.serialNumber(), exerciseDate_);
                exercised_ = exercised_ || exercise();
            }
        }
//    }
//    if (exercised_) {
        ...
        Real npv_ex = iIf((isPhysicalDelivery_ || today.serialNumber() == exerciseDate_)
...
//    } else {
        ...
        Real npv_no_ex = (isLong_ ? 1.0 : -1.0) * instrument_->NPV() * multiplier_;
//        return npv + addNPV;
//    }

    return iIf(exercised_, npv_ex, npv_no_ex) + addNPV;

}

QLBool EuropeanOptionWrapper::exercise() const {
...
QLBool AmericanOptionWrapper::exercise() const {
...
QLBool BermudanOptionWrapper::exercise() const {
    ...
    QLBool exercise = activeUnderlyingInstrument_->NPV() * undMultiplier_ > instrument_->NPV() * multiplier_;
    return exercise;
}
} // namespace data
} // namespace ore

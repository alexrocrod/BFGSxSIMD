...
LgmBuilder::LgmBuilder(int& aadc_i,   std::map<std::tuple<string, YieldCurveType, std::string>, AADCCurveData>& curves_data,
    ...
    : market_(market), configuration_(configuration), data_(data), bootstrapTolerance_(bootstrapTolerance),
      ...
      endCriteria_(EndCriteria(1000, 500, 1E-2, 1E-2, 1E-2)),
      calibrationErrorType_(BlackCalibrationHelper::RelativePriceError)
      , curves_data_(curves_data), use_aadc_(use_aadc), aadc_i_(aadc_i), aadc_initialized_(false), shift_(shift) {
...
void LgmBuilder::performCalculations() const {
    ...
    model_->setParams(params_);
        ...
        /*
        Real eps=0.000001;
        auto key = std::make_tuple("collateral_inccy", YieldCurveType::Discount, "EUR");
        if (use_aadc_ && !aadc_initialized_) { //nonsuc
                aadc_initialized_=true;
                idouble::CheckPoint();
                market_->markYieldCurvesAsInput(curves_data_);
        } 
        if (aadc_i_>=0) {
            market_->shift(aadc_i_, eps, key);
        }
        aadc_i_++; 
        */
    auto key = std::make_tuple("collateral_inccy", YieldCurveType::Discount, "EUR"); //!!!
    if (data_->calibrationType() != CalibrationType::None) {
        if (data_->calibrateA() && !data_->calibrateH()) {
            if (data_->aParamType() == ParamType::Piecewise && data_->calibrationType() == CalibrationType::Bootstrap) {
                ...
                model_->calibrateVolatilitiesIterative(aadc_i_, 
                    market_->discountCurve("EUR", "collateral_inccy"),
                    use_aadc_, shift_,  curves_data_[key].curve_args, 
...
void LgmBuilder::buildSwaptionBasket() const {
    ...
    for (Size j = 0; j < data_->optionExpiries().size(); j++) {
        ...
        if (termDateBased) {
            ...
            termTmp = static_cast<Size>(AAD_PASSIVE(termT + 0.5)) * Years;
        }
        ...
        if (expiryDateBased && termDateBased) {
            ...
            Real shift = svts_->volatilityType() == ShiftedLognormal ? svts_->shift(expiryDb, termT) : Real(0.0);
        ...
        if (expiryDateBased && !termDateBased) {
            ...
            Real shift = svts_->volatilityType() == ShiftedLognormal ? svts_->shift(expiryDb, termPb) : Real(0.0);
        ...
        if (!expiryDateBased && termDateBased) {
            ...
            Real shift = svts_->volatilityType() == ShiftedLognormal ? svts_->shift(expiryPb, termT) : Real(0.0);
        ...
        if (!expiryDateBased && !termDateBased) {
            ...
            Real shift = svts_->volatilityType() == ShiftedLognormal ? svts_->shift(expiryPb, termPb) : Real(0.0);

...
#if defined(AADC_QL) && defined(AADC_IDOUBLE_ACTIVE)
#define QL_AADC_ONLY_CHECK (!idouble::recording) &&
#else
#define QL_AADC_ONLY_CHECK 
#endif
/*! \def QL_ASSERT
    \brief throw an error if the given condition is not verified
*/
#define QL_ASSERT(condition,message) \
if (QL_AADC_ONLY_CHECK !bool(condition)) { \
    std::ostringstream _ql_msg_stream; \
    _ql_msg_stream << message; \
    throw QuantLib::Error(__FILE__,__LINE__, \
                          BOOST_CURRENT_FUNCTION,_ql_msg_stream.str()); \
...
/*! \def QL_REQUIRE_AAD
    \brief throw an error if the given pre-condition is not verified even during AADC recording
*/
#define QL_REQUIRE_AAD(condition,message) \
if (!(condition)) { \
    std::ostringstream _ql_msg_stream; \
    _ql_msg_stream << message; \
    throw QuantLib::Error(__FILE__,__LINE__, \

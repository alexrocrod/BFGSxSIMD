...
namespace QuantLib {
                      ...
                      Real displacement)
    {
        ...
        if (strike==0.0)
            return (optionType==Option::Call ? forward*discount : Real(0.0));
                               ...
                               Real discount)
    {
        ...
        stdDev =stdDev;
        //std::cout << MyDebugPrint::call_counter << "stdev\n";
        stdDev = std::max(1e-6, stdDev); //Zhenya
       
          //  return discount*std::max(d, 0.0);
        CumulativeNormalDistribution phi;
        Real result = discount*(stdDev*phi.derivative(d/stdDev) + d*phi(d/stdDev));
        QL_ENSURE(result>=0.0,
                  "negative value (" << result << ") for " <<
                  stdDev << " stdDev, " <<
                  optionType << " option, " <<
    ...
    Real bachelierBlackFormulaImpliedVol(Option::Type optionType,
        ...
        Real eta = iIf(std::fabs(nu) < SQRT_QL_EPSILON , 1.0 , nu / std::atanh(nu));

...
namespace QuantLib {
    ...
    std::complex<Real> BatesDoubleExpEngine::addOnTerm(
        ...
        const std::complex<Real> v1(p_/(1.0-g*nuUp_));
        const std::complex<Real> v2(q_/(1.0+g*nuDown_));
        const std::complex<Real> v3(p_/(1-nuUp_));
        const std::complex<Real> v4(q_/(1+nuDown_));
        const std::complex<Real> v5((v3 + v4-1.0));

        const std::complex<Real> res(t*lambda_*(v1 + v2 - 1.0
                          - g*v5));

        return t*lambda_*(v1 + v2 - 1.0
                          - g*v5);
    }

...
namespace QuantLib {
    ...
    void FDVanillaEngine::ensureStrikeInGrid() const {
        ...
        QLBool cond1(sMin_ > requiredGridValue/safetyZoneFactor_);
        sMin_ = iIf(cond1, requiredGridValue/safetyZoneFactor_, sMin_);
        sMax_ = iIf(cond1, center_/(sMin_/center_), sMax_);
...
//        }
        ...
        QLBool cond2(sMax_ < requiredGridValue*safetyZoneFactor_);
        sMax_ = iIf(cond2, requiredGridValue*safetyZoneFactor_, sMax_);
        sMin_ = iIf(cond2, center_/(sMax_/center_), sMin_);

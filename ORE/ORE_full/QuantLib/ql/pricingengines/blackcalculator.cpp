...
namespace QuantLib {
    ...
    void BlackCalculator::initialize(const ext::shared_ptr<StrikedTypePayoff>& p) {
        ...
        QLBool std_dev_cond(stdDev_>=QL_EPSILON);
        {
            {
                d1_ = QL_MAX_REAL;
                d2_ = QL_MAX_REAL;
            ...
            QLBool strike_not_zero_cond(!close(strike_, 0.0));
            {
                condAssign(d1_, strike_not_zero_cond, std::log(forward_/strike_)/stdDev_ + 0.5*stdDev_);
                condAssign(d2_, strike_not_zero_cond, d1_-stdDev_);
                condAssign(cum_d1_, strike_not_zero_cond, f(d1_));
                condAssign(cum_d2_, strike_not_zero_cond, f(d2_));
                condAssign(n_d1_, strike_not_zero_cond, f.derivative(d1_));
                condAssign(n_d2_, strike_not_zero_cond, f.derivative(d2_));
            }
        } //  else 
        {
            QLBool cond (!std_dev_cond && close(forward_, strike_)); 
            {
                condAssign(d1_,cond, 0);
                condAssign(d2_,cond, 0);
                condAssign(cum_d1_,cond, 0.5);
                condAssign(cum_d2_,cond, 0.5);
                condAssign(n_d1_,cond, M_SQRT_2 * M_1_SQRTPI);
                condAssign(n_d2_,cond, M_SQRT_2 * M_1_SQRTPI);
            } // else if 
            cond = !std_dev_cond && !close(forward_, strike_) && (forward_>strike_);
            {
                condAssign(d1_,cond, QL_MAX_REAL);
                condAssign(d2_,cond, QL_MAX_REAL);
                condAssign(cum_d1_,cond, 1.0);
                condAssign(cum_d2_,cond, 1.0);
                condAssign(n_d1_,cond, 0.0);
                condAssign(n_d2_,cond, 0.0);
            }// else 
            cond = !std_dev_cond && !close(forward_, strike_) && !(forward_>strike_);
            {
                condAssign(d1_,cond,QL_MIN_REAL);
                condAssign(d2_,cond,QL_MIN_REAL);
                condAssign(cum_d1_,cond,0.0);
                condAssign(cum_d2_,cond,0.0);
                condAssign(n_d1_,cond,0.0);
                condAssign(n_d2_,cond,0.0);
            }
        }

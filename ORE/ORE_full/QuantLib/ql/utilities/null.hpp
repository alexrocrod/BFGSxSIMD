...
namespace QuantLib {
    ...
    // AADC PATCH
    template<typename T>
    inline T operator -(const Null<T>& v) {
        return -T(v);
    }
#ifdef AADC_QL
    inline bool operator != (const idouble& a, const Null<idouble>& null) {
        // TODO: if recording, add check if this condition remains the same for all function calls
        return a.val != idouble(null).val; //  Null<double>();
    }
    inline bool operator == (const idouble& a, const Null<idouble>& null) {
        // TODO: if recording, add check if this condition remains the same for all function calls
        return a.val == idouble(null).val; //  Null<double>();
    }
    inline bool operator != (const BigNatural& a, const Null<idouble>& null) {
        // TODO: if recording, add check if this condition remains the same for all function calls
        return a != idouble(null).val; //  Null<double>();
    }
    inline bool operator == (const BigNatural& a, const Null<idouble>& null) {
        // TODO: if recording, add check if this condition remains the same for all function calls
        return a == idouble(null).val; //  Null<double>();

...
ZabrModel::fdPrice(const std::vector<Real> &strikes) const {
    ...
    const Size steps =
        (Size)stdpassive::ceil(expiryTime_ * 24); // number of steps in dimension t
...
Real ZabrModel::fullFdPrice(const Real strike) const {
    ...
    const Size steps = Size2(24 * expiryTime_ + 1);
    ...
    const Size sizefa = std::max<Size>(
        4, (Size)stdpassive::ceil(((x0 + x1) / 2.0 - f0) / (f1 - f0) * (Real)sizef));

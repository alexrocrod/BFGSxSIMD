    ...
    */
    ...
    : slope_(slope), K_(K), frequency_(anIndex->frequency()),
      ...
      capfloor_(MakeYoYInflationCapFloor(type,
            (Size)stdpassive::floor(0.5+surf->timeFromReference(surf->minMaturity())),
      ...
      priceToMatch_(priceToMatch), surf_(surf), p_(p) {
        ...
        Size n = (Size)stdpassive::floor(0.5 + surf->timeFromReference(surf_->minMaturity()));
        QL_REQUIRE( n > 0,
    ...
    initialize(const ext::shared_ptr<YoYCapFloorTermPriceSurface> &s,
        ...
        for (Size i=0; i<YoYCapFloorTermPriceSurface_->strikes().size(); i++) {
            ...
            for (Size j = 0; j < YoYCapFloorTermPriceSurface_->maturities().size(); j++){
                ...
                Size nT = (Size)stdpassive::floor(s->timeFromReference(s->yoyOptionDateFromTenor(Tp))+0.5);

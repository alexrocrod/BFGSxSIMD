...
namespace QuantLib {
    ...
    class OASHelper {
    public:
        OASHelper(const boost::function<Real(Real)>& npvhelper,
                  Real targetValue):
    ...
    private:
        const boost::function<Real(Real)>& npvhelper_;
        Real targetValue_;
    };
   ...
   Real CallableBond::NPVSpreadHelper::operator()(Real x) const
   {
    ...
    : CallableBond(settlementDays, schedule, accrualDayCounter,
        ...
        bool isZeroCouponBond = (coupons.size() == 1 && close_passive(coupons[0], 0.0));

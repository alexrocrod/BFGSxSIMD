...
namespace QuantLib {
    ...
    */
    ...
    Real RandomLM<D, C, URNG>::expectedShortfall(const Date& d,
        ...
        Real posit = stdpassive::ceil(percent * nSims_);
        posit = posit >= 0. ? posit : Real(0.);
        Size position = static_cast<Size>(Size2(posit));
    ...
    */
    ...
    boost::tuples::tuple<Real, Real, Real> // disposable?
        ...
        Size quantilePosition = static_cast<Size>(Size2(floor(nSims_*percentile)));
            ...
            Real operator()(Real t) const {
                ...
                */
                return dts_->defaultProbability(curveRef_ +
                    Period(static_cast<Integer>(Integer2(t)), Days), true) - pd_;
            }
          private:
        ...
        const std::vector<Real>& values) const
    {
        ...
        for(Size iName=0; iName<model_->size(); iName++) {
            ...
            if (horizonDefaultPs_[iName] >= simDefaultProb) {
                ...
                //  curve ref date:
                Size dateSTride =
                    static_cast<Size>(Size2(Brent().solve(// casted from Real:

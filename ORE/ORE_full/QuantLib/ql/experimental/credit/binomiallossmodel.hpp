    ...
    */
    ...
    {   // the model as it is does not model the exposures conditional to the 
        ...
        */
        ...
        Real floorAveProb = std::min(Real(bsktSize-1), stdpassive::floor(Real(m)));
        ...
        if(avgProb >= 1.-QL_EPSILON) {
            ...
            */
            ...
            lossProbDensity[Size2(floorAveProb)] += epsilon;
            lossProbDensity[Size2(ceilAveProb)]  += epsilonPlus;
        }
...
//            boost::lambda::_1 * boost::lambda::_2 / notBskt);
            [&](const Real& x, const Real& y) -> Real { return x * y / notBskt; });

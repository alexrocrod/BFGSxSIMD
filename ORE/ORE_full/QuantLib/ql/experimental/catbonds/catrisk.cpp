...
namespace QuantLib {
    ...
    namespace {
        Integer round(Real r) {
            return (r > 0.0) ? Integer(stdpassive::floor(r + 0.5)) : Integer(stdpassive::ceil(r - 0.5));
        }
    }
    ...
    BetaRiskSimulation::BetaRiskSimulation(Date start, Date end, Real maxLoss, Real lambda, Real alpha, Real beta) 
              : CatSimulation(start, end), 
                maxLoss_(maxLoss), 
                exponential_(rng_, boost::exponential_distribution<>(AAD_PASSIVE(lambda))),
                gammaAlpha_(rng_, boost::gamma_distribution<>(AAD_PASSIVE(alpha))),
                gammaBeta_(rng_, boost::gamma_distribution<>(AAD_PASSIVE(beta)))
    {

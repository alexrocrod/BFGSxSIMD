...
namespace QuantLib {
    ...
    Real CumulativeNormalDistribution::operator()(Real z) const {
        ...
        if (idouble::recording) {
            Real res = aadc::cdf_normal(z); // (1.0 + std::erf(z* M_SQRT_2)) * 0.5;
            return res;
        }
#endif
//        return res; // TODO: Check if asymptotic expansion is required

//        __m512d mmres = _mm512_cdfnorm_pd(_mm512_set1_pd(z.val));

//        Real boost_res = boost::math::cdf(
//            boost::math::normal_distribution<double>(0.0, 1.0), z.val
//        );
...
//        return boost_res;
    ...
    Real MaddockInverseCumulativeNormal::operator()(Real x) const {
        return AADBOOST::math::quantile(
            AADBOOST::math::normal_distribution<Real>(average_, sigma_), x);
    }

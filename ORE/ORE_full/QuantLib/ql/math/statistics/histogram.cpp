    ...
    namespace {
        ...
        */
        Real quantile(const std::vector<Real>& samples, Real prob) {
            ...
            Size index = static_cast<Size>(stdpassive::floor((nsample+a)*prob+a));
            ...
            switch (algorithm_) {
              case Sturges: {
                  bins_ = static_cast<Size>(
                           stdpassive::ceil(std::log(static_cast<Real>(data_.size()))
                                     /std::log(2.0) + 1));
                  break;
              }
              case FD: {
                  ...
                  bins_ = static_cast<Size>(stdpassive::ceil((max-min)/h));
                  break;
              }
              case Scott: {
        ...
        if (breaks_.empty()) {
            ...
            std::vector<Real>::iterator end =
                std::unique(breaks_.begin(),breaks_.end(),
                            static_cast<bool (*)(Real, Real)>(close_enough_passive));

...
namespace QuantLib {
    ...
    void IncrementalStatistics::add(Real value, Real valueWeight) {
        QL_REQUIRE(valueWeight >= 0.0, "negative weight (" << valueWeight
                                                           << ") not allowed");
        acc_(AAD_PASSIVE(value), boost::accumulators::weight = AAD_PASSIVE(valueWeight));
        if(value < 0.0)
            downsideAcc_(AAD_PASSIVE(value), boost::accumulators::weight = AAD_PASSIVE(valueWeight));
    }

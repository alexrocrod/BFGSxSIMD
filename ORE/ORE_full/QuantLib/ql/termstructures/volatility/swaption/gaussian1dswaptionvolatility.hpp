    ...
    class DateHelper {
      public:
        ...
        Real operator()(Real date) const {
            Date d1(static_cast<Date::serial_type>(AAD_PASSIVE(date)));
            Date d2(static_cast<Date::serial_type>(AAD_PASSIVE(date)) + 1);
            Real t1 = ts_.timeFromReference(d1) - t_;
            Real t2 = ts_.timeFromReference(d2) - t_;
            Real h = date - static_cast<Date::serial_type>(Size2(date));

...
#include <vector>
    ...
    typedef __m256d mmType;
#endif
inline std::shared_ptr<aadc::AADCFunctions<mmType>> getAADC() {
    static  auto aad_funcs2 = std::shared_ptr<aadc::AADCFunctions<mmType> >(new aadc::AADCFunctions<mmType>());
    return aad_funcs2;
}
...
namespace QuantLib {
    ...
    struct AADCCurveData {
        aadc::VectorArg curve_args;
        std::vector<Real> curve_vals;
        std::vector<Time> curve_times;
        std::vector<double> CVA_diffs;
    };
    ...
    class YieldTermStructure : public TermStructure {
      public:
        ...
        */
        ...
        virtual void markVecAsInput(aadc::VectorArg& vector_arg) {
            throw "Yield curve is not marked!!!\n";
        }
        virtual void markVecAsOutput(aadc::VectorRes& vector_arg) {
            
        }
        virtual void CurveUpdate () {throw "Yield curve is not marked!!!\n";}
        virtual std::vector<Real>& data () {throw "method was not redefined";}
        virtual std::vector<Real>& times () {throw "method was not redefined";}
        virtual void initializeInstruments () {throw "method was not redefined";}

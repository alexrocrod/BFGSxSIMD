  ...
  class InterpolatedPiecewiseZeroSpreadedTermStructure : public ZeroYieldStructure {
    public:
      InterpolatedPiecewiseZeroSpreadedTermStructure(
        ...
        std::vector<Real>& data () {return originalCurve_->data();}
        std::vector<Real>& times () {return originalCurve_->times();}
        void markVecAsInput (aadc::VectorArg& vector_arg) {
            //originalCurve_->markVecAsInput(vector_arg);
            this->updateInterpolation();
        }
        void CurveUpdate() {this->updateInterpolation();}

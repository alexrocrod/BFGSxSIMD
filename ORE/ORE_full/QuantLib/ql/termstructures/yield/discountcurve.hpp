...
namespace QuantLib {
    ...
    std::vector<Real>& data () {return this->data_;}
    std::vector<Real>& times () {return this->times_;}
    void markVecAsInput (aadc::VectorArg& vector_arg) {
        for (int i=0; i< this->data_.size(); i++) {
            vector_arg.push_back(this->data_[i].markAsInput());
            // vector_arg.push_back(this->data_[i].markAsDiff());
    ...
    void markVecAsOutput (aadc::VectorRes& vector_arg) {
        for (int i=0; i< this->data_.size(); i++) {
            vector_arg.push_back(this->data_[i].markAsOutput());
        }
    }
    ...
    void CurveUpdate() {
        this->interpolation_ =
            this->interpolator_.interpolate(this->times_.begin(),
                                            ...
                                            this->data_.begin());
        this->interpolation_.update();
        std::cout << "SIZE " << this->interpolation_.logY().size() << "\n";
        //for (int i=0; i<33; i++) std::cout << i <<this->interpolation_.logY()[i] << "\n"; 
        //this->interpolation_.logY()[31]=this->interpolation_.logY()[31];

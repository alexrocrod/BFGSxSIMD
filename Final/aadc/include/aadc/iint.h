...
public:
    ...
    iint& operator *= (const iint& other) {
        iint tmp;
        if(AADC_UNLIKELY(idouble::recording)) CAAD_iIntVarMult(&val, &other.val, &tmp.val);
        val = tmp.val;
    ...
    iint& operator += (const iint& other) {
        iint tmp;
        if(AADC_UNLIKELY(idouble::recording)) CAAD_iIntVarAdd(&val, &other.val, &tmp.val);
        val = tmp.val;
        ...
        iint& operator -= (const iint& other) {
        iint tmp;
        if(AADC_UNLIKELY(idouble::recording)) CAAD_iIntVarSub(&val, &other.val, &tmp.val);
        val = tmp.val;
...
iint lower_bound(_ForwardIterator __first, _ForwardIterator __last, const idouble& x)
{
    ...
    if(AADC_UNLIKELY(idouble::recording)) {
        ...
        size_t step = size > 1 ? &(*next) - &(*__first) : 1;

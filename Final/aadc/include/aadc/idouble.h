...
extern "C" AADC_API uint64_t CAAD_CheckPoint();
...
namespace std {
    template<>
    class numeric_limits<idouble> : public numeric_limits<double> {
    public:
        static idouble infinity();
    };
}
...
//        if(AADC_UNLIKELY(recording)) CAAD_iVarAddAssign(&(other.val), &val);
    ...
    operator int() const { 
#ifdef AADC_IDOUBLE_ACTIVE        
        if(idouble::recording && CAAD_iVarIsRandom(&val)) {
            CAAD_iVarExtractPassive(&val, "idouble -> bool at UNKNOWN Location. Use AADC_BreakOnActiveBoolConversion option", 0);
        }
#endif
        return int(val);
    }
    ...
    aadc::AADCArgument markAsInputPy() const {
        return this->markAsInput();
    }
    ...
    aadc::AADCResult markAsOutputPy() const {
        return this->markAsOutput();
    }
...
public:
    static uint64_t CheckPoint() {
        if(AADC_UNLIKELY(recording)) return CAAD_CheckPoint();
        return 0;
    }
...
inline idouble operator op (type a, itype b) { \
    return idouble(double(a)) op b; \
} \
inline idouble operator op (itype a, type b) { \
    return a op idouble(double(b)); \
}
...
AADC_MIX_BINARY_OP(op, long, itype) \
AADC_MIX_BINARY_OP(op, unsigned long, itype)
#else
#define AADC_MIX_BINARY_OP_ALL(op, itype) \
AADC_MIX_BINARY_OP(op, float, itype) \
AADC_MIX_BINARY_OP(op, double, itype) \
AADC_MIX_BINARY_OP(op, int, itype) \
AADC_MIX_BINARY_OP(op, unsigned int, itype) \
AADC_MIX_BINARY_OP(op, int64_t, itype) \
AADC_MIX_BINARY_OP(op, uint64_t, itype) \
AADC_MIX_BINARY_OP(op, long long, itype)
#endif
...
inline idouble pow(const idouble& x, int y) {
    return AADC_OPS_NAMESPACE::pow(x, idouble(y));
}
...
inline long lround(const idouble& x) {
    return long (round(x).val); 
}

inline bool isnan(const idouble& x) {
    return std::isnan(x.val); 
}

inline bool isinf(const idouble& x) {
...
inline idouble cdf_normal(const idouble& x, idouble& pdf)
{
    idouble res;
...
inline idouble cdf_normal(const idouble& x)
{
    idouble res, pdf;
    res.val = cdf_normal(x.val, pdf.val);
    if (AADC_UNLIKELY(idouble::recording)) CAAD_iVarCDFNorm(&x.val, &res.val, &pdf.val);
...
inline uint64_t breakIfRecording(const idouble* m) {

    if (idouble::recording) throw "TODO";
    return 0;
}

inline uint64_t breakIfRecording(const double* m) {
    return 0;
}

#define AAD_PASSIVE_PTR(m) ((double*)m + breakIfRecording(m))
...
namespace std {
    ...
    inline idouble modf(const idouble& x, idouble* y) {
        return modf(AAD_PASSIVE(x), AAD_PASSIVE_PTR(y));
        // TODO!
    }
...
namespace std {
   inline idouble numeric_limits<idouble>::infinity() { return numeric_limits<double>::infinity(); }
}

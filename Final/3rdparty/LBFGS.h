...
public:
    ...
    inline std::pair <int,int> minimize(Foo& f, Vector& x, Scalar& fx, bool reuse = false )
    {
        using std::abs;
        ...
        // Added: reuse previous Hessian approximation
        if (reuse && n!=m_xp.size()){
            std::cout << "WARNING: reuse not possible - different dimensions\n";
            reuse = false;
        }

        if (!reuse || !m_used) reset(n); // do not reset the old values
        ...
        for( ; ; )
        {
            ...
            niter_LS += LineSearch<Scalar>::LineSearch(f, m_param, m_xp, m_drt, step_max, step, fx, m_grad, dg, x);
